...
单文件 CLI，同时是 Nonoka script 工具（stdin 传 JSON、stdout 出纯文本或 JSON）。
...
```
bilibili_live_stream   单文件脚本(CLI + Nonoka 工具)
index-entry.json       Nonoka src/scripts/index.json 的注册项(尚未安装)
```
