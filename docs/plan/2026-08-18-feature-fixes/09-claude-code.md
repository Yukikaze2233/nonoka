...
- Nonoka 当前只有自己的 provider/LLM 客户端，没有任何调用本地 `claude` CLI 的工具。
...
- `task` 是 Nonoka 自己的子代理循环，tier 语义是“模型档位”；Claude Code 是外部 agent runtime，输入输出、权限、会话续跑、成本模型都不同，混在一起会让两者都难维护。
...
```json
{
  ...
  "parameters": {
    "type": "object",
    "properties": {
      "prompt": {"type": "string", "description": "Complete task prompt for Claude Code. Include required context; the CLI does not see Nonoka's conversation unless explicitly included."},
...
2. 参数：`-p --output-format json --permission-mode <configured>`；`cwd` 为会话 workspace；环境继承当前 Nonoka daemon（含 HOME，才能读到订阅登录态）。
...
- 不把 Claude 用量塞进 Nonoka `Usage`（那是 OpenAI 兼容 usage 口径）。新增独立统计：`claude_code` 调用写入隐藏审计会话（类似 subagent），字段含 `cost_usd`、`duration_ms`、模型、session id、prompt 长度。
...
1. PATH 放一个假 `claude` 脚本（fixture），断言 Nonoka 调用参数、cwd、env、超时 kill、JSON 解析与截断。
...
2. **`nonoka mcp-serve`**（隐藏子命令，`src/cli/mcp_serve.rs`）：MCP stdio server,
   与 `nonoka tool-call` 同源——daemon 存活走 IPC ToolCatalog/ToolCall（会话→模式
   →registry,guard 管线齐备）,直连本地兜底。`ToolCatalog` 新增 `full` 位一次拿
   全量合同。供应商自动以 `--mcp-config` 挂桥（env 显式带 NONOKA_SESSION/
   NONOKA_TURN_ORIGIN/NONOKA_HOME/XDG_RUNTIME_DIR）。
...
用户实测发现经桥的 Nonoka 工具在 claude 侧执行、本就不走 Nonoka 渲染管线,"保
渲染"不成立;裁定原生工具转正。定稿:

- **两个独立四档作用域**(off/dev/normal/all,按会话模式裁决,专用表单可改):
  `native_tools`(claude 自带 Bash/Edit/Read…,**默认 all**)与
  `nonoka_tools`(MCP 桥,**默认 off**)。可叠加:dev 会话可同时拥有两边。
...
1. **清空联动**:续传映射织入 Nonoka 会话 id(不同会话显式隔离,字节级撞链也
...
2. **上下文限制**:窗口不钉值(第五轮用户裁定:回猜测默认 168k,要改自己在模型菜单设);
   Nonoka 的压缩管自己的账本,claude 侧真实上下文由其 autocompact 自管,Nonoka
   压缩→链断→重开会话间接同步。
...
5. 表单标签去掉档位括号。
6. **会话隔离**:见 1,映射按 (provider, model, nonoka 会话) 三元隔离。

另:双四档默认改 native=all + nonoka=normal;供应商列表 Claude Code 置顶
(default_templates + normalize 搬移,四个位置式引用测试改按 id 定位)。

**已知未修**(桥语义,与中转正交):经桥调用 Nonoka `task` 的子代理工具集为空
(用户实测);ask_question 未过桥。`task` 已进去重剔除表,中转路径不受影响。
**排查铁律**:隔离测试 home 的配置文件会钉住旧默认(permission_mode/
nonoka_tools 两次踩坑)——"默认没生效"先查配置残值。
...
- **后台任务跟进**:claude 自己的后台/通知活在单次进程里,轮末即杀跟不了
  进;job/alarm 从去重表**请回**(Nonoka 的 daemon 常驻+完成唤醒才是这套架构
  的后台);glob/grep/todowrite 按用户裁定入表。nonoka_tools 默认改 **all**
...
- **task 经中转的子代理"空工具集"真相**:中转按设计丢弃外层工具定义
  (工具循环不外交),SubagentRunner 的 Nonoka 工具对内层 claude 不可见;此前
...
- **task 从去重表请回**:与 claude 原生 Task 语义不同——Nonoka 子代理在
...
- **中转环境事实注入**(常量字节):每轮一进程、自带后台/通知活不过本轮;
  桥在场时补 mcp__nonoka__task/job/alarm 三句。第八轮 task 只改了注释没删数
...
- MCP 对外接入达成度:`nonoka mcp-serve` 是标准 stdio server,任何 MCP 客户
  端可挂(env: NONOKA_SESSION 指会话;NONOKA_HOME/XDG_RUNTIME_DIR 必须与
  daemon 环境一致地传或不传)。
