...
装上可选组件 `nonoka-voice`,设置里开「语音功能」,daemon 会拉起一个独立进程
常开麦克风:喊「未有未有」→ 提示音 + 通知「Nonoka 在听」→ 说指令 → 通知「Nonoka
收到:…」→ 她在「语音会话」里执行 → 完成后提示音 + 通知回复摘要。终端 REPL
`/stt`、`nonoka stt`、WebUI 麦克风按钮三个入口共用同一套识别做**听写**。

## 二、为什么是两个可执行

| | `nonoka`(主程序) | `nonoka-voice`(语音前端) |
|---|---|---|
...
连接、语音回合驱动、通知、听写中继。`nonoka-voice` 只懂音频:麦克风 → 能量门 →
VAD → 唤醒词 → 识别 → 信令;它以普通 IPC 客户端连回 daemon,daemon 消失即退出。

```
nonoka(daemon) ──spawn──> nonoka-voice
   ...
   · voice.dictation{text} → 听写认领者(REPL /stt、nonoka stt)
   · voice.transcribe{wav_path} ← WebUI 浏览器录音
```

Cargo:`voice` feature **默认关**;`[[bin]] nonoka-voice` 标 `required-features`。
`cargo build --release` 只出 nonoka;`cargo build --release --features voice` 两个都出。
打包:`packaging/arch/nonoka-release` 拆成 `nonoka` + `nonoka-voice` 两个包,sherpa 静态库
作为 source 由 makepkg 下载校验(`SHERPA_ONNX_ARCHIVE_DIR`),构建期不联网;
AUR 包装包 `packaging/arch/nonoka-voice`。
...
| 假名 / 拉丁 | `诺娜诺娜`、`nonokanonoka` | 假名→罗马音→按日语音节映射到近似拼音(ゆ 按 you 而非 yu),声调未知就同一个词展开成轻声+一到四声五行候选,任一命中即算 |
...
音色 5/5 命中;`nonokanonoka` / `诺娜诺娜` 按中文口音念(识别成"米有米游")命中,
...
| 事件 | 提示音 | 桌面通知 |
|---|---|---|
| 唤醒词命中 | wake(上行两音) | 「Nonoka 在听 / 请讲」(与提示音同一瞬间) |
| 识别出指令 | heard(单点) | 「Nonoka 收到 / <指令>」 |
| 回合完成 | done(下行三音) | 「Nonoka / <回复前 N 字>」(N=`notify_reply_chars`) |
| `nonoka listen` 再按一次关闭 | off(下行两音,wake 的镜像) | 「Nonoka / 不听了」 |
| 回合失败 | error(低音) | 「语音会话出错 / …」 |
| 窗口关闭/超时 | 无 | 无 |

**语音会话**:唤醒对话落在一条专属会话(kind = `voice`,id 记在
`state/voice-session-id`),不进 WebUI 列表;用 `nonoka voice history [--limit n]`
回看、`nonoka voice reset` 清空(下次唤醒重建)、`nonoka voice status` 看前端状态。
...
(装上 nonoka-voice、填 key、开开关三步即可)。TUI 里在某家填了 key 而当前那家没
key,会自动切过去;两家都有 key 时在「播报供应商」列表按 Tab 切换。合成好的 wav
由 daemon 交给 nonoka-voice 播放;播报期间麦克风帧丢弃(没有回声消除,半双工),
`nonoka listen` 会先掐掉播报再收听。
...
缺东西一律静默:没装 `nonoka-voice` 只在 daemon 日志 warn 一次;没填 key 时开关
照样能开,填上的那一刻生效;QQ 语音转写不可用时留占位不报错。

两个独立开关:`voice.enabled` = **语音唤醒**(麦克风常开、唤醒词、听写、
`nonoka listen`),`voice.tts.enabled` = **文本转语音**(回复播报、`speak` 工具)。
任一开启都会拉起 nonoka-voice;唤醒关闭时它不开麦克风、不下载识别模型,只管播放。
...
`nonoka voice say "文本"` / WebUI `POST /api/voice/tts/preview` 同样可试听;WebUI 设置页
...
(要 wav)→ 交给 nonoka-voice 转写 → `[语音] 文本`,模型、主动回复判官、历史库看
的都是这份。**要语音唤醒开着**(识别模型跟它加载)且前端已接上;没开、没装、
取不到文件、转写失败一律静默退化成 `[语音消息]` 占位,不弹通知不报错。

**快捷键呼叫**:`nonoka listen` 是个**开关**:她没在听时让前端直接进入等待指令
...
例如 niri:`Mod+Space { spawn "nonoka" "listen"; }`。
...
`testkit/voice/sounds/synth.py` 生成,内嵌进 nonoka-voice),`voice.sounds` 开关、
`voice.sound_volume` 音量,`nonoka-voice cue done` 试听。
...
| `nonoka stt` | 本机麦 | 第一句即提交为消息,前台流式打印回复(shellhook 形态) |
| WebUI 麦克风按钮 | 浏览器 `getUserMedia` → 前端重采样 16k PCM16 → WebSocket `/api/voice/stream` 持续推给 daemon → 转给 `nonoka-voice`(VAD/分句/识别与本机麦同一套) | 识别一句回一句,逐句填进输入框;输入框上方悬浮麦克风电平指示;静默 10s 自动结束,再点麦克风或 Esc 也能结束 |

浏览器麦克风只在 https 或 localhost 可用;LAN http 页面按钮会提示改用 `/stt`。
`POST /api/voice/transcribe`(整段 WAV → 文本)保留给外部脚本,WebUI 不再用它。

听写认领是全局单例:REPL、`nonoka stt`、WebUI 同一时刻只能有一个在听写。
浏览器流式听写期间 `nonoka-voice` 丢弃本机麦克风的帧,结束后恢复唤醒监听。
...
"voice": {
  "enabled": false,
  "wake_keywords": ["未有未有", "密友密友", "nonokanonoka", "诺娜诺娜"],   // 可多个,任一命中即唤醒;写逗号分隔的字符串也行
  "wake_threshold": 0.25, "wake_boost": 1.0,
  "microphone": null,                 // TUI/WebUI 从 `nonoka-voice devices` 列表里选;null=系统默认
  "stt_threads": 2,
...
改动经 `nonoka reload` / WebUI 保存生效:voice 节变了 daemon 就重启前端进程。

## 七、排查

- 状态:`GET /api/voice/status`(二进制在不在、是否 attached、采集设备、日志路径),
  IPC `VoiceStatus` 同源。
- 日志:`logs/voice-worker.log`(`NONOKA_VOICE_DEBUG=1` 打各阶段耗时)。
- **"没反应"先跑 `nonoka-voice test --keyword 未有未有 --timings`**:一行"听到语音"
  都没有 = 音频没进来(查 `nonoka-voice devices` / 系统默认源);有但不命中 = 唤醒词
  层(试调低阈值 / 换声调错开的词)。
- 前端找不到:daemon 日志 warn「找不到 nonoka-voice」;放到 nonoka 同目录或 PATH。
- 麦克风占用冲突:同机只跑一个 nonoka-voice;调试时 `NONOKA_HOME` 隔离的 daemon
  也会拉起自己的一份。
- **提示音/播报全无、喊了没反应,但 `nonoka-voice test` 单跑正常(09-06 事故)**:先看
  WirePlumber 有没有把 nonoka-voice 的流"记住"到别的设备——
  `grep nonoka-voice ~/.local/state/wireplumber/stream-properties`,任何带 `"target"`
  的条目都可疑(用 `pactl move-*`/pavucontrol 挪过一次流,WirePlumber 就按
  `application.name` 永久记住,之后每条 "PipeWire ALSA [nonoka-voice]" 流都落到那
  里;测试夹具把播放流挪进 null sink 就是这么把线上提示音弄没的)。清法:
  `systemctl --user stop wireplumber` → 删掉那几行里的 `"target":...` → start →
  重启 nonoka-voice(`nonoka reload`)。再看 `pw-dump` 里 `alsa_capture.nonoka-voice` 的
  Link 是不是接在你真正说话的那只麦上;两只全速 USB 声卡挂同一个 hub 时,一只被
  常开会让另一只 `dmesg: Not enough bandwidth for altsetting` 起不来,把配置
  `voice.microphone` 写成 `nonoka-voice devices` 里那只麦的源名,别让它跟着"默认源"漂。
...
- 唤醒词实验台:`NONOKA_KW_WAVS=<wav目录> NONOKA_KW_KEYWORDS=a,b NONOKA_KW_EACH=1
  cargo test --features voice --lib voice::e2e_tests::keyword_lab -- --ignored --nocapture`
  输出 关键词×样本 命中矩阵(`NONOKA_KW_STT=1` 附识别文本;`kws_raw` 绕开管线直接喂模型,
...
- **绝不用 `pactl move-sink-input` / `move-source-output` 挪 nonoka-voice 的流**:WirePlumber
...
v2(`nonoka-voice test --timings`,同一夹具、同一批 test_wavs,机器同样有并行编译;
...
`stt_unload_seconds` 兜底;二进制:`nonoka` 58.1MB(与无语音版持平),`nonoka-voice` 35.9MB。
