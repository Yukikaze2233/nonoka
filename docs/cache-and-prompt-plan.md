# Hotaru 改进方案 v7（定稿）：高缓存命中率 + 精简提示词

> 五轮流程的最终定稿：四轮 Claude 研究（广度 → Hotaru 对抗验证 → pi/Reasonix 实现深潜+供应商核查 → v3 落地性核查）+ 第五轮对两份外部 review 的裁定合并：
...
| R7 tokPerChar 自校准 | **不采纳为默认**：Hotaru 已内嵌精确 o200k BPE（token_counter.rs）。留作注记 |
| R1（字节稳定性基建前置）/R3（TRANSIENT 正则自动生成+幂等测试）/R4（探针快照瞬态合并）/R6（压缩逐字保留地板）/R12（辅助请求独立 scheduler 状态+重试复用自身前缀）/P1（工具三件套与静态化同批）/P2（hotaru.md 审查表交付物）/P4（辅助隔离测试断言）/P7（append-only 工程规约）/P8（deferred tools 并列 experiments）/§〇遗漏 2（cooldown 纳入亲和判定）/遗漏 6（responses continuation.endpoint_id 锚点复用） | **全部采纳**（归属见正文） |
...
| persona/identity 变更"下会话生效"→"当前 turn 冻结、下一 turn 原子切换 generation" | **采纳**（Hotaru 会话长驻） |
...
2. **hotaru.md 去留表**（GPT v6 §5.2 为基准草案，实施时逐条确认）：人格/关系/语气核心保留压实；对话示例缩到高区分度少量；"逻辑自检/固定 80-90% 置信/每问必双搜"删除或改触发式；todo/loader 教程、AUR 流程、游戏数据源顺序、计算器强制 → 下沉到工具 snippet/guideline 或状态机；易过期 Linux 事实 → KB 加时间戳；外貌/喜好 → 按需 appendix；**Emoji 全禁 vs AUR/游戏要求彩色 Emoji 的冲突 → 实施时请用户定夺统一规则**。
...
| 存量修复 | token 向量测试的两个过期值（上游改 hotaru.md/README 未更新） | token_estimate.rs |
...
- **eval 门控**：hotaru.md 瘦身（Release 3a 要求离线 eval 先行）、工具三件套 snippet/guideline 全量填充。
...
**新增方案 ①（采纳，列为 Release 2c 的推荐演进 "1.3-c stub 模式"）**：全部懒工具以 **stub 常驻**——保留真名 + 一句话描述 + 宽松参数壳 `{arguments: object}`，常驻一个 `load_tool_schemas` 工具按需以 tool result 返回完整 schema（追加在对话尾部、不碰前缀）。文中估算 200 工具 80k→15k（-80%）。对 Hotaru 的适配性极佳：
...
- Hotaru 执行层本就不做 schema 校验（tools 核查 §3 已确认 handler 自行取参），"内层校验下放客户端"对 Hotaru 是零损失；
