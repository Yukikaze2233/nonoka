...
方法：运行项目测试套件 + rustc 探针程序（release 语义，逐字复刻涉事代码）+ 真实 release 二进制沙箱黑盒触发（`NONOKA_HOME` 指向临时目录）。全部中间产物已清理。
...
首次黑盒探测（`alarm --help`）因未设置 `NONOKA_HOME`，被 REPL 当作用户消息路由到了正在运行的生产 daemon，产生了**一轮真实对话**及少量 usage 记录（`~/.nonoka/state/conversation.db-wal`、`usage-history.jsonl` 有对应写入）。未删除任何数据（删除用户数据风险大于收益）；如需清理可在 WebUI 中删除该条 `alarm --help` 会话。此后全部动态测试均在 `/tmp` 沙箱内进行。中间产物（探针程序、沙箱目录、核心转储）已全部删除；`target/` 内测试编译产物属正常运行缓存。
...
7. **`src/cli.rs:436`** — `extract_debug_flag` 扫描 `--` 之前的全部参数：`nonoka 你好 --debug` 的消息正文被截成"你好"并意外开 debug。
...
18. **`src/shell/fish.rs:162`** — `trap` 是 fish 3.6+ builtin：老 fish 下变未知命令 → 触发自己的 command_not_found 钩子 → **把 `trap __nonoka_restore_cursor ...` 当用户消息发给 AI**（每次两次），光标恢复失效。
