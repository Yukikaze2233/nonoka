...
只发增量)。会花两次真实 agy 调用(gemini flash),不进 CI;验收手跑:
    ...
    NONOKA_HOME=/tmp/nonoka-agy/home python3 testkit/antigravity/run.py

前提:NONOKA_HOME 下 config.jsonc 的 active_provider 是 antigravity 协议供应商,本机 agy 已登录,daemon 未运行(直连互斥)。
