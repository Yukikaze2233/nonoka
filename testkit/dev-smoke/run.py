...
PTY 进 `nonoka dev` → 你好一轮 → 断言:无「找不到该会话」、会话挂 dev 人格、
...
class DevRepl(persona_ab.Repl):
    """persona-ab 的 PTY 驱动,换成 daemon 形态 `nonoka dev`。"""
...
def main() -> int:
    assert NONOKA.exists(), f"先 cargo build:缺 {NONOKA}"
    build_home()
