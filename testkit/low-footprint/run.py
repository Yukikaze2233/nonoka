...
用法:
    BIN=/usr/bin/hotaru python3 run.py baseline
    BIN=../../target/release/hotaru python3 run.py round2

隔离手法(见 memory/hotaru-live-testing-notes):独立 HOTARU_HOME + 独立
...
REPO = BASE.parents[1]
BIN = Path(os.environ.get("BIN", REPO / "target" / "release" / "hotaru")).resolve()
...
REAL_CONFIG = Path.home() / ".hotaru" / "config" / "config.jsonc"
REAL_MODELS_CACHE = Path.home() / ".hotaru" / "cache" / "models_cache.json"
...
def env_for(extra=None):
    env = dict(os.environ)
    for k in ("XDG_CACHE_HOME", "XDG_CONFIG_HOME", "XDG_DATA_HOME", "XDG_STATE_HOME", "HOTARU_DIRECT"):
        env.pop(k, None)
    env["HOTARU_HOME"] = str(HOME)
