...
HOTARU_BIN = REPO / "target" / "debug" / "hotaru"
...
class Repl:
    ...
    def __init__(self, home: Path, log: Path):
        ...
        # 裸 hotaru 自②批起打印模式帮助退出;测具显式进普通模式。
        self.proc = subprocess.Popen(
            [str(HOTARU_BIN), "normal"],
            stdin=slave,
            stdout=slave,
            stderr=slave,
            env=env,
...
def score_chat(reply: str) -> dict:
    """默认 Hotaru 人格显式规则的机械化子集(与 08-14 判据同源):
