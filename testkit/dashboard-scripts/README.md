...
```sh
# 1. 隔离 home（不要用真实 ~/.nonoka；config 里不配 web 密码即免登录）
H=/tmp/nonoka-dash-scripts
...
NONOKA_HOME=$H XDG_RUNTIME_DIR=$H/run NONOKA_SYSTEM_SCRIPTS_DIR=$PWD/src/scripts \
  target/debug/nonoka __daemon --port 18402 &
# 3. 走查
python3 testkit/dashboard-scripts/shoot.py http://127.0.0.1:18402
```

坑：`XDG_RUNTIME_DIR` 路径太长会撞 `SUN_LEN`；daemon 用 `__daemon` 隐藏子命令直接起，别用 `nonoka web`（它会去找/拉线上 daemon）。改了 JS/CSS 必须重新 `cargo build` 再重启，静态资源编进二进制。
