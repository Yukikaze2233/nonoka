...
"""语音 v2 全链 e2e(隔离环境,不碰线上 daemon、不碰真实麦克风):
  ...
  隔离 NONOKA_HOME + 隔离 XDG_RUNTIME_DIR(IPC socket 不与 8300 撞)
  桩 LLM(OpenAI 兼容 SSE)← daemon(__daemon --port) → nonoka-voice(同目录)
  PipeWire null sink 注入模型自带 test_wavs;stream.capture.sink 让采集口
  自动接到 sink 的 monitor(不会接到真实麦克风)
  假 notify-send 前置到 PATH,记录每条桌面通知

验证点:
  1. daemon 拉起 nonoka-voice 并 attach(voice.status attached=true)
...
需要: ~/.nonoka/state/models 已有模型(或 NONOKA_VOICE_MODELS_DIR)。
"""
...
NONOKA = BIN_DIR / "nonoka"
VOICE = BIN_DIR / "nonoka-voice"
assert NONOKA.is_file() and VOICE.is_file(), f"缺二进制: {NONOKA} / {VOICE}"

WORK = Path("/tmp/nonoka-voice-e2e")
HOME = WORK / "home"
RUN_DIR = WORK / "run"
FAKEBIN = WORK / "bin"
...
SINK = "nonoka_test_sink"
KWS = Path(os.environ.get("NONOKA_VOICE_MODELS_DIR", Path.home() / ".nonoka/state/models")) / \
...
daemon = subprocess.Popen([str(NONOKA), "__daemon", "--port", str(WEB_PORT), "--bind", "127.0.0.1"], env=env, stdout=daemon_log, stderr=subprocess.STDOUT, preexec_fn=os.setsid)
results = {}

def socket_path():
    """NONOKA_HOME 隔离时运行目录叫 nonoka-<hash>,用 glob 找。"""
...
try:
    ...
    # 1. worker attach
    ok = wait_for(lambda: http("GET", "/api/voice/status").get("attached") is True, 60, "nonoka-voice attach")
    ...
    ok = wait_for(lambda: any(line.startswith("-a\tNonoka\t--\t未有\t") for line in notifications()), 30, "完成通知")
    ...
    if marker.exists():
        ...
        # 语音会话是 voice kind,不走 WebUI 的会话接口;用 `nonoka voice history` 读。
        history = subprocess.run([str(NONOKA), "voice", "history", "--limit", "5"], env=env, capture_output=True, text=True, timeout=30)
