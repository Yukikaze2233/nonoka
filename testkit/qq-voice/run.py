...
daemon 调 get_record 拿 wav(假 NapCat 回 base64)→ nonoka-voice 转写 → 正文变成
`[语音] 文本` → 桩 LLM 回复。语音关着时应静默退化成 `[语音消息]` 占位。

隔离:NONOKA_HOME + XDG_RUNTIME_DIR;桩 LLM;nonoka-voice 的麦克风被 PIPEWIRE_NODE
...
env.update(NONOKA_HOME=str(HOME), XDG_RUNTIME_DIR=str(RUN_DIR), LANG="zh_CN.UTF-8", NONOKA_LOG="info",
...
def api_data(action, params):
    ...
    if action == "get_login_info": return {"user_id": SELF_ID, "nickname": "Nonoka"}
...
def pump(ws):
    while True:
        ...
        if action in ("send_group_msg", "send_msg", "send_private_msg"):
            REPLIES.append(params.get("message")); say(f"← Nonoka 回复: {render(params.get('message'))}")
        else:
...
try:
    ...
    if not args.no_voice:
        assert wait_for(lambda: http("/api/voice/status").get("attached") is True, 60, "nonoka-voice attach")
        time.sleep(1)
    ...
    say("→ 发了一条纯语音私聊消息")
    got = wait_for(lambda: len(REPLIES) > 0, 90, "Nonoka 回复")
    results["replied"] = got
