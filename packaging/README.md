...
- `arch/nonoka/` — AUR 二进制包装包（下载上述 Release 资产 + Noto 字体），
  AUR `nonoka` 的真相源
...
3. `gh release create vX.Y.Z <产物>.pkg.tar.zst --title "Nonoka X.Y.Z"`
4. 更新 `arch/nonoka/PKGBUILD` 的 `pkgver` 与资产 sha256，
...
| `/usr/share/nonoka/scripts/` | 系统级脚本 | `src/scripts/` | 无内置脚本 |
