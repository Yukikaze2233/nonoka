...
impl PersonaMenuTarget {
    ...
    pub(in crate::config_tui) fn is_hotaru(&self, config: &AppConfig) -> bool {
        match self {
            Self::Global => config.prompt.active_persona.trim().is_empty(),
            Self::Platform(persona) => matches!(persona, PlatformPersonaOverride::Hotaru),
        }
    }
    ...
    pub(in crate::config_tui) fn activate_hotaru(&mut self, config: &mut AppConfig) {
        match self {
            Self::Global => config.prompt.active_persona.clear(),
            Self::Platform(persona) => *persona = PlatformPersonaOverride::Hotaru,
        }
    }
...
pub(in crate::config_tui) fn manage_personas(
    ...
    loop {
        ...
        options.push(format!(
            "{}Hotaru",
            if target.is_hotaru(config) { "* " } else { "  " }
        ));
...
pub(in crate::config_tui) fn persona_aux_fields(
    ...
    let (hint_label, dialogs_label) = if hotaru {
        (
            t(
...
/// 默认 Hotaru 人格:本体只读,回车只编辑附属的防失忆提示与预设对话
/// (scope 固定为 default)。
pub(in crate::config_tui) fn edit_hotaru_persona_extras(
    stdout: &mut io::Stdout,
    paths: &HotaruPaths,
    config: &AppConfig,
) -> Result<()> {
    let (hint, dialogs) = crate::persona_hint::hotaru_aux_prefill(config, paths);
    let mut fields = persona_aux_fields(hint, dialogs, true);
    if !run_form(stdout, t(" HOTARU EXTRAS ", " Hotaru 人格附加 "), &mut fields)? {
        return Ok(());
    }
