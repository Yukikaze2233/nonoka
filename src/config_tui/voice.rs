...
//!
//! 唤醒词能否编码成 KWS 音节这件事在 `nonoka-voice` 进程里做(本进程不链接
//! 语音栈),这里只做形式校验(非空、含中文)。麦克风列表同样问
//! `nonoka-voice devices` 要,二进制不在就退化成手填。
//!
//! TUI 跑在 `nonoka config` 的 tokio 运行时线程上,不能在这里 `block_on`
...
/// 问 `nonoka-voice devices` 要输入源列表(源名, 描述);拿不到返回空。
...
fn preview_tts(
    ...
    block_on_thread(move || async move {
        let mut stream = crate::ipc::connect(&socket).await.context(t(
            "Nonoka daemon is not running (preview needs it)",
            "Nonoka daemon 未运行(试听要 daemon 在跑)",
        ))?;
        crate::ipc::send(
            &mut stream,
...
fn edit_mimo(stdout: &mut io::Stdout, paths: &NonokaPaths, config: &mut AppConfig) -> Result<()> {
    let mut selected = 0usize;
    loop {
...
fn edit_voice_form(stdout: &mut io::Stdout, config: &mut AppConfig) -> Result<()> {
    ...
    let mic_field = if devices.is_empty() {
        Field::new(
            t(
                ...
                "麦克风(空=系统默认;装了 nonoka-voice 才能列设备)",
            ),
    ...
    for keyword in &keywords {
        ...
        // nonoka-voice 按模型词表判定,编不出来的它会记日志跳过。
        ...
        if !acceptable {
            bail!(t(
                ...
                "唤醒词只能是汉字、假名或拉丁字母(如 未有未有 / みゆみゆ / nonokanonoka)"
            ));
        }
    }
