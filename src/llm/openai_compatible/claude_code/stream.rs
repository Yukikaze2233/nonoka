...
//! 消息累积,权威用量以最终 `result` 帧为准;②tool_use 不回吐给 Hotaru 执行
    ...
    F: FnMut(ChatStreamChunk) -> Result<()>,
{
    ...
    for block in blocks {
        ...
        // MCP 前缀剥掉:Hotaru 工具按本名显示(readable_tool_name 才认识),
        // claude 原生工具保持原名。
        let name = raw_name.strip_prefix("mcp__hotaru__").unwrap_or(raw_name);
