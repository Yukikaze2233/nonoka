...
//! 既有的订阅登录态,Nanoka 不经手任何凭据。工具循环的所有权在 claude 侧——
//! Nanoka 的工具经 `nanoka mcp-serve` 桥挂进去(内层调用照走 daemon 的 guard 管
//! 线),所以这条线对 Nanoka 的回合循环呈现为「一次请求、纯文本(+思考)回来、
//! 永远没有 tool_calls」。
//!
//! 会话续传:Nanoka 的消息历史是严格 append-only 的(缓存体制的成果),所以
...
pub(in crate::llm::openai_compatible) struct ClaudeCodeRuntime {
    ...
    /// Nanoka 工具经 MCP 桥挂给 claude 的模式作用域:off/dev/normal/all。
    pub(in crate::llm::openai_compatible) nanoka_tools: String,
    /// 原生工具开启时的 --permission-mode(无头模式没有交互审批)。
    pub(in crate::llm::openai_compatible) permission_mode: String,
    /// 每个 (provider\tmodel) 的 --autocompact 阈值:取 Nanoka 的有效窗口值
    /// (显式配置→目录→默认 168k),夹到 CLI 接受的 100k–1M。claude 在这个
    /// 尺寸自压缩,会话 id 不变、续传不断——窗口语义单一来源是 Nanoka 配置。
...
impl ClaudeCodeRuntime {
    pub(in crate::llm::openai_compatible) fn from_config(config: &AppConfig) -> Self {
        ...
        Self {
            ...
            nanoka_tools: plugin.nanoka_tools.clone(),
...
impl OpenAiCompatibleClient {
        ...
        F: FnMut(ChatStreamChunk) -> Result<()>,
    {
        ...
        let nanoka_session = crate::tools::workspace::try_session();
        let nanoka_session = nanoka_session.as_deref();
    ...
    fn claude_code_args(
        ...
        // 思考档:Nanoka 的 thinking-variant 选择映射到 CLI 的 --effort。
        ...
        let nanoka_on = tool_capable && scope_allows(&runtime.nanoka_tools, self.claude_code_dev_mode);
        {
            ...
            if native_on || nanoka_on {
                prompt.push_str(RELAY_ENVIRONMENT_NOTE);
                if nanoka_on {
                    prompt.push_str(RELAY_NANOKA_TOOLS_NOTE);
                }
            }
        ...
        if nanoka_on {
            // 两套同开时去重:与 claude 原生重复的 Nanoka 工具剔除,原生优先
...
/// Nanoka 工具经 MCP stdio 桥挂给 claude:`nanoka mcp-serve` 打回 daemon,与
/// `nanoka tool-call` 同一条会话→模式→registry 解析链。没有会话作用域(测试
/// /直连辅助请求)就不挂桥。
/// 中转环境事实(声明式,不写指令;常量字节保证前缀稳定)。
const RELAY_ENVIRONMENT_NOTE: &str = "\n\n<relay-environment>\nThis session runs inside Nanoka's relay: each turn is a fresh CLI process that exits when the turn ends. Work backgrounded through the built-in tools (Bash run_in_background, background Task) dies with the process, and its completion notifications never arrive.\n</relay-environment>";

/// nanoka 工具桥在场时的补充事实。
const RELAY_NANOKA_TOOLS_NOTE: &str = "\n<relay-environment-tools>\nThe mcp__nanoka__ tools live in the persistent Nanoka daemon and survive across turns: mcp__nanoka__task runs a background subagent that wakes a follow-up turn when it finishes, mcp__nanoka__job inspects or stops those, and mcp__nanoka__alarm schedules timed reminders.\n</relay-environment-tools>";

/// 两套工具同开时从桥里剔除的 Nanoka 工具(与 claude 原生功能重复,原生
/// 在训练分布内、优先)。task **不剔**:与原生 Task 语义不同——Nanoka 子代理
/// 在 daemon 里作为后台任务运行、完成后唤醒开新轮跟进,与 job(查/停)成对。
/// job/alarm **不剔**:claude 自己的后台/定时机制
/// 活在单次进程里,中转每轮一进程、轮末即杀,活不过回合;Nanoka 的 job 走
...
fn mcp_bridge_config(exclude_duplicates: bool) -> Option<String> {
    ...
    let exe = crate::paths::nanoka_executable().ok()?;
    let origin =
        serde_json::to_string(&crate::tools::workspace::current_turn_origin()).ok()?;
    let mut env = serde_json::Map::new();
    env.insert("NANOKA_SESSION".into(), json!(&*session));
    env.insert("NANOKA_TURN_ORIGIN".into(), json!(origin));
    if exclude_duplicates {
        env.insert(
            "NANOKA_MCP_EXCLUDE".into(),
            json!(BRIDGE_DUPLICATE_TOOLS.join(",")),
        );
    }
    // claude 给 MCP server 的是洁净环境,home/runtime 识别变量要显式带——
    // 但必须**如实透传**(daemon 自己有什么才给什么):runtime 目录推导对
    // "显式设了 NANOKA_HOME"与"没设"给出不同路径(默认 home 显式设也会变
    // 成哈希子目录),无条件塞 NANOKA_HOME 会让 mcp-serve 连不上正常启动的
    // daemon,静默滑进直连兜底(实测:目录缺 WebUI 工具、图片打进
    // mcp-serve 自己的 stdout、资产全无)。
    for key in ["NANOKA_HOME", "XDG_RUNTIME_DIR"] {
...
/// 清空 Nanoka 会话时的联动:丢弃它名下的续传映射,并尽力删除 claude 侧的
/// 会话转录(`~/.claude/projects/<项目槽>/<会话id>.jsonl`)。存储布局是
/// claude 的内部实现,删不到只记日志不报错——映射已丢弃,该 claude 会话
/// 无论如何不会再被续传。
pub(crate) fn forget_claude_code_session(nanoka_session: &str) {
    let removed = session::forget_nanoka_session(nanoka_session);
    if removed.is_empty() {
        return;
    }
        ...
        for claude_session in &removed {
            ...
            match std::fs::remove_file(&transcript) {
                Ok(()) => tracing::info!(
                    ...
                    "removed the claude-side transcript for a cleared Nanoka session"
                ),
