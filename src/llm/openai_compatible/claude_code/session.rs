//! claude 会话与 Nanoka 消息前缀的对应关系(进程内)。
//!
//! 键是「逐消息哈希链」:chain[i] = 前 i 条会话消息的链哈希,种子掺入
//! provider/model/system prompt。Nanoka 的历史回放是字节级 append-only 的,
...
struct SessionEntry {
    provider_id: String,
    model: String,
    /// 归属的 Nanoka 会话(workspace task-local):续传匹配显式按它隔离,
    /// 清空 Nanoka 会话时按它联动丢弃。回合作用域外(直连兜底)为 None。
    nanoka_session: Option<String>,
...
pub(super) fn find_resumable(
    ...
    sessions
        ...
        .filter(|entry| entry.nanoka_session.as_deref() == nanoka_session)
        .filter(|entry| {
...
pub(super) fn record_session(
    ...
    sessions.push(SessionEntry {
        ...
        nanoka_session: nanoka_session.map(str::to_string),
        prefix_len,
        prefix_hash,
        claude_session,
    });
...
/// 清空 Nanoka 会话时联动:丢弃它名下的全部映射,返回对应的 claude 会话 id
/// (调用方拿去做 claude 侧转录的尽力删除)。
pub(in crate::llm::openai_compatible) fn forget_nanoka_session(nanoka_session: &str) -> Vec<String> {
    let Ok(mut sessions) = SESSIONS.lock() else {
        return Vec::new();
    };
    let mut removed = Vec::new();
    sessions.retain(|entry| {
        if entry.nanoka_session.as_deref() == Some(nanoka_session) {
...
mod tests {
    ...
    fn prefix_matching_follows_append_only_history() {
        ...
        record_session("p", "m", Some("nanoka-a"), 2, chain[2], "sess-1".to_string());
        ...
        assert_eq!(
            find_resumable("p", "m", Some("nanoka-a"), &chain, extended.len()),
            Some(("sess-1".to_string(), 2))
        );

        // 别的 Nanoka 会话即使字节级同前缀,也绝不共用 claude 会话。
        assert_eq!(
            find_resumable("p", "m", Some("nanoka-b"), &chain, extended.len()),
            None
        );
        ...
        assert_eq!(
            find_resumable("p", "m", Some("nanoka-a"), &chain, rewritten.len()),
            None
        );
        ...
        let chain = prefix_chain("p", "m", "sys", &base);
        assert_eq!(
            find_resumable("p", "m", Some("nanoka-a"), &chain, base.len()),
            None
        );

        // 清空 Nanoka 会话 ⇒ 名下映射整体丢弃,并交回 claude 会话 id。
        assert_eq!(forget_nanoka_session("nanoka-a"), vec!["sess-1".to_string()]);
        let chain = prefix_chain("p", "m", "sys", &extended);
        assert_eq!(
            find_resumable("p", "m", Some("nanoka-a"), &chain, extended.len()),
            None
