...
mod tests {
    ...
    fn prefix_matching_follows_append_only_history() {
        ...
        record_session("p", "m", Some("nonoka-a"), 2, chain[2], "sess-1".to_string());
        ...
        assert_eq!(
            find_resumable("p", "m", Some("nonoka-a"), &chain, extended.len()),
            Some(("sess-1".to_string(), 2))
        );

        // 别的 Nonoka 会话即使字节级同前缀,也绝不共用 claude 会话。
        assert_eq!(
            find_resumable("p", "m", Some("nonoka-b"), &chain, extended.len()),
            None
        );
        ...
        assert_eq!(
            find_resumable("p", "m", Some("nonoka-a"), &chain, rewritten.len()),
            None
        );
        ...
        let chain = prefix_chain("p", "m", "sys", &base);
        assert_eq!(
            find_resumable("p", "m", Some("nonoka-a"), &chain, base.len()),
            None
        );
