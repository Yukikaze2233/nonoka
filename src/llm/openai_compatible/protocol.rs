...
impl ThinkingVariantPreferences {
    pub(crate) fn load(paths: &NonokaPaths) -> Self {
        Self::load_for_update(paths).unwrap_or_default()
    }

    pub(in crate::llm::openai_compatible) fn load_for_update(paths: &NonokaPaths) -> Result<Self> {
    ...
    pub(crate) fn save(&self, paths: &NonokaPaths) -> Result<()> {
