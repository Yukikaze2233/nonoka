//! Moving a Hotaru installation between machines: `hotaru export` / `hotaru import`.
...
pub(crate) mod tests {
    ...
    /// A HotaruPaths rooted at `root`, mirroring the real layout.
    pub(crate) fn test_paths(root: &Path) -> HotaruPaths {
        HotaruPaths {
    ...
    /// Everything Hotaru writes under `HOTARU_HOME` must be classified.
    ///
    ...
    fn populated_home(root: &Path) -> HotaruPaths {
        ...
        std::fs::write(root.join("cache/logs/hotaru.log"), "noise").unwrap();
    ...
    fn an_export_round_trips_into_an_empty_home() {
        ...
        let archive = out.path().join("hotaru-export.tar.gz");
        ...
        assert!(!target.path().join("cache/logs/hotaru.log").exists());
    ...
    fn every_data_location_is_classified() {
        ...
        let observed = [
            ".layout-v1",
            ".resource-layout-v1",
            "cache/logs/hotaru.2026-08-08.log",
        ...
        assert!(
            ...
            "unclassified paths under HOTARU_HOME: {unclassified:?}\n\
