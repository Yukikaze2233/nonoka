...
/// 用户看到的就是 `hotaru` 启动卡死两分钟。超时了就跳过这轮检查——它只是
...
pub fn ensure_initialized(paths: &HotaruPaths, config: &AppConfig) -> Result<()> {
...
pub fn status(paths: &HotaruPaths) -> Result<DefaultKbStatus> {
    let state = load_state(paths)?;
    Ok(DefaultKbStatus {
...
pub fn notice_if_update_available(paths: &HotaruPaths) -> Result<Option<String>> {
    ...
    let message = t(
        "The default knowledge base needs an update; run hotaru update-default-kb",
        "默认知识库需要更新，运行 hotaru update-default-kb",
    )
    .to_string();
...
pub async fn check_update_if_due(paths: &HotaruPaths) -> Result<()> {
    let mut state = load_state(paths)?;
...
fn default_kb_source_dir() -> PathBuf {
    std::env::var_os("HOTARU_DEFAULT_KB_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("/usr/share/hotaru/default-kb"))
}

fn state_file(paths: &HotaruPaths) -> PathBuf {
    paths.data_dir.join("default-kb/state.json")
}

fn update_repo_dir(paths: &HotaruPaths) -> PathBuf {
    paths
        .cache_dir
        .join("default-kb/shorin-archlinux-guide.git")
}

fn legacy_update_repo_dir(paths: &HotaruPaths) -> PathBuf {
    paths.cache_dir.join("default-kb/shorinwiki.git")
}

fn update_source_dir(paths: &HotaruPaths) -> PathBuf {
    paths.cache_dir.join("default-kb/update-source")
}

fn cleanup_legacy_update_repo(paths: &HotaruPaths, repo: &Path) -> Result<()> {
...
fn load_state(paths: &HotaruPaths) -> Result<DefaultKbState> {
...
fn save_state(paths: &HotaruPaths, state: &DefaultKbState) -> Result<()> {
    let path = state_file(paths);
...
fn build_update_source(paths: &HotaruPaths, repo: &Path) -> Result<PathBuf> {
