...
impl MemoryOrganizer {
    pub(crate) fn spawn() -> Result<Self> {
        ...
        let join = std::thread::Builder::new()
            .name("nonoka-memory-organizer".to_string())
...
impl MemoryOrganizerHandle {
    pub(crate) fn wake(&self, config: AppConfig, paths: NonokaPaths, state_store: StateStore) {
...
mod tests {
    ...
    fn test_paths(temp: &tempfile::TempDir) -> NonokaPaths {
        NonokaPaths {
