...
fn completion_entries() -> [(&'static str, &'static str); 16] {
    [
        ...
        (
            "reload",
            t(
                "Reload configuration in the running Nonoka daemon",
                "在运行中的 Nonoka daemon 内重新加载配置",
            ),
        ),
        ...
        (
            "remove-shell-hook",
            t(
                "Remove installed Nonoka shell hooks",
                "安全删除已安装的 Nonoka shell hook",
            ),
        ),
        ...
        (
            "update-default-kb",
            t("Update the default knowledge base", "更新 Nonoka 默认知识库"),
        ),
        (
            "memory",
...
pub fn hook() -> String {
    ...
    for (command, description) in completion_entries() {
        output.push_str(&format!(
            "complete -c nonoka -n __fish_use_subcommand -f -a {command} -d '{description}'\n"
        ));
    }
    output.push('\n');
    output.push_str(
        r#"function __nonoka_paste
    set -l output (nonoka --clipboard-paste 2>/dev/null)
    if test $status -eq 0; and test -n "$output"
        if not set -q __nonoka_image_counter
...
bind \cv __nonoka_paste

function __nonoka_insert_newline
    commandline -f expand-abbr
    commandline -i \n
end

bind ctrl-j __nonoka_insert_newline
bind \cj __nonoka_insert_newline
bind -M insert ctrl-j __nonoka_insert_newline
bind -M insert \cj __nonoka_insert_newline

function __nonoka_wrap_fish_prompt
    functions -q __nonoka_original_fish_prompt; and return
    functions -q fish_prompt; or fish_prompt >/dev/null 2>/dev/null
    functions -q fish_prompt; or return

    functions -c fish_prompt __nonoka_original_fish_prompt
    function fish_prompt
        if set -q __nonoka_pending_buffer
            printf '\e[?25l'
        end
        __nonoka_original_fish_prompt
    end
end

function __nonoka_replay_buffer
    set -l buffer $argv[1]
...
function __nonoka_restore_cursor
    printf '\e[?25h'
    set -e __nonoka_cursor_hidden
end

function __nonoka_on_prompt --on-event fish_prompt
    set -q __nonoka_pending_buffer; or return

    set -l buffer $__nonoka_pending_buffer
    set -e __nonoka_pending_buffer
    set -e __nonoka_image_counter

    trap __nonoka_restore_cursor INT TERM EXIT
    __nonoka_replay_buffer "$buffer"
    printf '\n'
    printf '%s' "$buffer" | nonoka --shell-intercept --shell fish --stdin
    set -l nonoka_status $status
    trap - INT TERM EXIT
    __nonoka_restore_cursor
    return $nonoka_status
end

function __nonoka_execute_or_continue
...
function __nonoka_buffer_is_multiline
    test (string split \n -- "$argv[1]" | count) -gt 1
end

function __nonoka_first_command
...
function __nonoka_accept_line
    ...
    if test -z "$trimmed"
        __nonoka_execute_or_continue
        return
    end

    if not __nonoka_buffer_is_multiline "$buffer"
        __nonoka_execute_or_continue
        return
    end

    set -l first_command (__nonoka_first_command "$buffer")
    ...
    printf '%s' "$buffer" | nonoka --shell-classify --shell fish --stdin 2>/dev/null
    ...
    set -e __nonoka_image_counter
    __nonoka_wrap_fish_prompt
    set -g __nonoka_cursor_hidden 1
    history append -- "$buffer"
    set -g __nonoka_pending_buffer "$buffer"
    commandline -b -- ""
    printf '\e[?25l'
    commandline -f execute
end

bind enter __nonoka_accept_line
bind \r __nonoka_accept_line
bind -M insert enter __nonoka_accept_line
bind -M insert \r __nonoka_accept_line
...
function fish_command_not_found
    ...
    if test -n "$current_line"; and not string match -qr '[\n\r]' -- "$current_line"
        set -l top_command (__nonoka_first_command "$current_line")
        if test -z "$top_command"; or not type -q -- "$top_command"
            printf '\n'
            printf '%s' "$current_line" | nonoka --shell-intercept --shell fish --stdin 2>/dev/null
            return 127
        end
    end
    ...
    nonoka --shell-intercept --shell fish -- $command 2>/dev/null
    return 127
end
"#,
    );
...
mod tests {
    ...
    fn fish_hook_defines_command_not_found_handler() {
        ...
        assert!(hook.contains("printf '%s' \"$current_line\" | nonoka --shell-intercept"));
    ...
    fn fish_hook_defines_curated_top_level_completions() {
        ...
        let completion_lines = hook
            .lines()
            .filter(|line| line.starts_with("complete -c nonoka "))
            .collect::<Vec<_>>();
    ...
    fn fish_hook_defines_paste_binding() {
        let hook = hook();
        assert!(hook.contains("__nonoka_paste"));
        assert!(hook.contains("bind \\cv __nonoka_paste"));
        assert!(hook.contains("nonoka --clipboard-paste"));
    }

    #[test]
    fn fish_hook_defines_enter_binding() {
        let hook = hook();
        assert!(hook.contains("__nonoka_accept_line"));
        assert!(hook.contains("__nonoka_wrap_fish_prompt"));
        assert!(hook.contains("functions -c fish_prompt __nonoka_original_fish_prompt"));
        assert!(hook.contains("if set -q __nonoka_pending_buffer"));
        assert!(hook.contains("__nonoka_replay_buffer"));
        assert!(hook.contains("__nonoka_on_prompt --on-event fish_prompt"));
        assert!(hook.contains("__nonoka_replay_buffer \"$buffer\"\n    printf '\\n'"));
        ...
        assert!(hook.contains("set -g __nonoka_cursor_hidden 1"));
        assert!(hook.contains("set -e __nonoka_cursor_hidden"));
        assert!(hook.contains("return $nonoka_status"));
        assert!(hook.contains("__nonoka_execute_or_continue"));
        assert!(hook.contains("__nonoka_buffer_is_multiline"));
        assert!(hook.contains("test (string split \\n -- \"$argv[1]\" | count) -gt 1"));
        assert!(hook.contains("__nonoka_first_command"));
        assert!(hook.contains("commandline --input=\"$argv[1]\" --tokens-expanded"));
        assert!(hook.contains("type -q -- \"$first_command\""));
        assert!(hook.contains("set -g __nonoka_pending_buffer \"$buffer\""));
        ...
        assert!(!hook.contains("__nonoka_multiline_has_unknown_command"));
        assert!(hook.contains("--shell-classify --shell fish --stdin"));
        assert!(hook.contains("--shell-intercept --shell fish --stdin"));
        assert!(hook.contains("bind enter __nonoka_accept_line"));
        assert!(hook.contains("bind \\r __nonoka_accept_line"));
        assert!(hook.contains("bind ctrl-j __nonoka_insert_newline"));
        assert!(hook.contains("bind -M insert enter __nonoka_accept_line"));
        assert!(hook.contains("bind -M insert ctrl-j __nonoka_insert_newline"));
    }

    #[test]
    fn fish_hook_resets_image_counter_on_command_not_found() {
        let hook = hook();
        assert!(hook.contains("set -e __nonoka_image_counter"));
    }
    ...
    fn uninstall_reports_only_existing_hook() {
        ...
        let paths = NonokaPaths {
            ...
            fish_hook_file: temp.path().join("nonoka.fish"),
