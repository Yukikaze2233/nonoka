...
const BEGIN_MARKER: &str = "# >>> hotaru bash hook >>>";
const END_MARKER: &str = "# <<< hotaru bash hook <<<";
...
pub fn hook() -> &'static str {
    ...
    hotaru --shell-intercept --shell bash -- "$@" 2>/dev/null
    return 127
}
"#
}
...
mod tests {
    ...
    fn installing_again_refreshes_an_existing_hook_path() {
        ...
        std::fs::write(
            &rc_path,
            format!("before\n{BEGIN_MARKER}\nsource '/old/hotaru-hook.sh'\n{END_MARKER}\nafter\n"),
        )
        .unwrap();
        let hook = temp.path().join("new hotaru-hook.sh");
        ...
        assert!(updated.contains("new hotaru-hook.sh"));
        assert!(!updated.contains("/old/hotaru-hook.sh"));
