...
/// Shared between the daemon (startup banner) and the CLI (`nonoka web` /
/// `--status` output).
...
pub fn stage_managed_web_password(paths: &NonokaPaths, password: &str) -> Result<PathBuf> {
...
pub fn stage_web_password_file(paths: &NonokaPaths, source: &Path) -> Result<PathBuf> {
...
pub(crate) fn load_daemon_launch_config(paths: &NonokaPaths) -> Result<DaemonLaunchConfig> {
...
pub(crate) fn abandon_daemon_launch_candidate(paths: &NonokaPaths, config: &DaemonLaunchConfig) {
...
pub(crate) fn remove_managed_password(paths: &NonokaPaths, path: &Path) {
...
pub(crate) fn write_private_state(path: &Path, contents: &[u8]) -> Result<()> {
    let parent = path.parent().context("Nonoka state file has no parent")?;
...
pub(crate) fn finish_private_state_commit(parent: &Path, directory_sync: std::io::Result<()>) -> Result<()> {
    if let Err(error) = directory_sync {
        tracing::warn!(
            ...
            error = %error,
            "Nonoka state file was committed, but syncing its parent directory failed"
        );
    }
    Ok(())
}
...
pub(crate) fn recover_legacy_daemon_launch(paths: &NonokaPaths, pid: u32) -> Result<DaemonLaunchConfig> {
    ...
    let cwd = std::fs::read_link(format!("/proc/{pid}/cwd"))
        .context("reading legacy Nonoka daemon working directory")?;
...
pub fn discard_daemon_launch_candidate(paths: &NonokaPaths, config: &DaemonLaunchConfig) {
