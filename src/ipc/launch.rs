...
/// Shared between the daemon (startup banner) and the CLI (`nanoka web` /
/// `--status` output).
...
pub fn stage_managed_web_password(paths: &NanokaPaths, password: &str) -> Result<PathBuf> {
...
pub fn stage_web_password_file(paths: &NanokaPaths, source: &Path) -> Result<PathBuf> {
...
pub(crate) fn try_load_daemon_launch_config(paths: &NanokaPaths) -> Result<Option<DaemonLaunchConfig>> {
...
pub(crate) fn load_daemon_launch_config(paths: &NanokaPaths) -> Result<DaemonLaunchConfig> {
...
pub(crate) fn save_daemon_launch_config(paths: &NanokaPaths, config: &DaemonLaunchConfig) -> Result<()> {
    let path = paths.daemon_launch_state_file();
...
pub(crate) fn commit_daemon_launch_config(paths: &NanokaPaths, config: &DaemonLaunchConfig) -> Result<()> {
...
pub(crate) fn abandon_daemon_launch_candidate(paths: &NanokaPaths, config: &DaemonLaunchConfig) {
...
pub(crate) fn remove_managed_password(paths: &NanokaPaths, path: &Path) {
...
pub(crate) fn write_private_state(path: &Path, contents: &[u8]) -> Result<()> {
    let parent = path.parent().context("Nanoka state file has no parent")?;
...
pub(crate) fn finish_private_state_commit(parent: &Path, directory_sync: std::io::Result<()>) -> Result<()> {
    if let Err(error) = directory_sync {
        tracing::warn!(
            ...
            error = %error,
            "Nanoka state file was committed, but syncing its parent directory failed"
        );
    }
    Ok(())
}
...
#[cfg(target_os = "linux")]
pub(crate) fn recover_legacy_daemon_launch(paths: &NanokaPaths, pid: u32) -> Result<DaemonLaunchConfig> {
    let cmdline = std::fs::read(format!("/proc/{pid}/cmdline"))
        .context("reading legacy Nanoka daemon arguments")?;
    let cwd = std::fs::read_link(format!("/proc/{pid}/cwd"))
        .context("reading legacy Nanoka daemon working directory")?;
...
pub(crate) fn recover_legacy_daemon_launch(_paths: &NanokaPaths, _pid: u32) -> Result<DaemonLaunchConfig> {
...
pub fn discard_daemon_launch_candidate(paths: &NanokaPaths, config: &DaemonLaunchConfig) {
