...
pub const ADMIN_BUSY_MESSAGE: &str = "Nonoka is busy with another operation";
...
pub const BUILD_ID: &str = env!("NONOKA_BUILD_ID");
...
mod tests {
    ...
    fn daemon_process_prefers_the_default_web_port_unless_overridden() {
        let mut default = std::process::Command::new("nonoka");
        ...
        let mut overridden = std::process::Command::new("nonoka");
    ...
    fn bare_launch_restores_the_saved_port_and_password_file() {
        ...
        let mut command = std::process::Command::new("nonoka");
    ...
    fn legacy_inline_password_and_port_are_recovered_into_managed_state() {
        let temp = tempfile::tempdir().unwrap();
        let paths = test_paths(temp.path());
        let cmdline = b"/usr/bin/nonoka\0__daemon\0--port\09412\0--password=legacy-secret\0";
    ...
    fn legacy_relative_password_file_is_copied_from_the_daemon_cwd() {
        ...
        let cmdline = b"nonoka\0__daemon\0--port=9500\0--password-file\0external-password\0";
