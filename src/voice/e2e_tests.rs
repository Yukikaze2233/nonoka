...
//! 需要模型已就位(~/.nonoka/state/models 或 NONOKA_VOICE_MODELS_DIR),
...
fn models_dir() -> Option<PathBuf> {
    let dir = std::env::var_os("NONOKA_VOICE_MODELS_DIR")
        .map(PathBuf::from)
        .or_else(|| {
            directories::BaseDirs::new().map(|base| base.home_dir().join(".nonoka/state/models"))
        })?;
...
/// `nonoka listen` 是开关:空闲时进入等待指令(Wake),已在听时关窗(ListenOff),
/// 再按又进入等待指令;听写窗口内忽略。
#[test]
#[ignore = "需要本地语音模型"]
...
/// 唤醒词实验台:`NONOKA_KW_WAVS=<目录> NONOKA_KW_KEYWORDS=a,b,c` 逐个 wav 过
...
fn keyword_lab() {
    ...
    let Ok(wavs_dir) = std::env::var("NONOKA_KW_WAVS") else {
        eprintln!("未设置 NONOKA_KW_WAVS,跳过");
        return;
    };
    let keywords: Vec<String> = std::env::var("NONOKA_KW_KEYWORDS")
        .unwrap_or_else(|_| "未有未有".to_string())
        .split(',')
        .map(|item| item.trim().to_string())
        .filter(|item| !item.is_empty())
        .collect();
    let threshold: f32 = std::env::var("NONOKA_KW_THRESHOLD")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(0.25);
    let boost: f32 = std::env::var("NONOKA_KW_BOOST")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(1.0);
    ...
    let with_stt = std::env::var_os("NONOKA_KW_STT").is_some();
    ...
    // NONOKA_KW_EACH=1:每个唤醒词单独建管线,输出 keyword×wav 命中矩阵。
    if std::env::var_os("NONOKA_KW_EACH").is_some() {
