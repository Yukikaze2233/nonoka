...
//! |---|---|---|
//! | 汉字 | `未有未有` | 每字注带调拼音,一行 |
//! | 假名 / 拉丁 | `みゆみゆ`、`nonokanonoka` | 假名先转罗马音,再按日语音节切分映射到近似拼音;声调未知,同一个词展开成轻声 + 一到四声几条候选行,任一命中即唤醒 |
//! | 显式拼音 | `mi1 yu2 mi1 yu2` | 空格分隔的"拼音+声调数字"(0 或不带数字 = 轻声),原样编码 |
//!
//! 一个词里可以混写(`小nonoka`):汉字定调,其余部分展开声调候选。模型是普通话
//! 模型,非中文只是近似发音;能不能稳定命中要靠 `nonoka-voice test` 实测。
...
/// 拉丁输入也走这张表:`nonokanonoka` → mi yu mi yu。
fn romaji_syllables() -> &'static [(&'static str, &'static [&'static str])] {
    &[
        ...
        // nonokanonoka 被识别成"米有米游",KWS 也只认 mi/you 的组合)。
        ("ya", &["ya"]),
        ("yu", &["you", "yu"]),
        ("yo", &["you"]),
        ("ra", &["la"]),
...
fn romaji_to_syllables(latin: &str) -> Result<Vec<String>> {
    ...
    while !rest.is_empty() {
        ...
        let Some((romaji, candidates)) = hit else {
            bail!("「{rest}」无法按音节切分(支持日语罗马音写法,如 nonoka、shiro)");
        };
...
mod tests {
    ...
    fn latin_expands_tone_variants() {
        ...
        let lines = encode_keyword("nonokanonoka", file.path()).unwrap();
        assert_eq!(lines.len(), 5, "{lines:?}");
        assert_eq!(lines[0], "m ī y ū m ī y ū @nonokanonoka");
        assert_eq!(lines[4], "m i y u m i y u @nonokanonoka");
    }
    ...
    fn mixed_han_and_latin() {
        ...
        assert_eq!(
            encode_keyword("小nonoka", file.path()).unwrap(),
            vec!["x iǎo m ī y ū @小nonoka"]
        );
    }
    ...
    fn rejects_unsupported_characters() {
        ...
        assert!(looks_encodable("nonokanonoka"));
