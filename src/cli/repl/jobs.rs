...
///
/// `focused` is `None` when there is no live tail — a one-shot `nonoka ask` has
...
pub(in crate::cli) fn spawn_jobs_poll_thread(paths: NonokaPaths) -> std::sync::Arc<SharedJobsFeed> {
