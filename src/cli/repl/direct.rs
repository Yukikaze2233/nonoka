//! 进程内直连的回合驱动。
//!
//! `HOTARU_DIRECT=1` 或 daemon 起不来时走这条：agent 直接在本进程跑，事件不过
    ...
    loop {
        ...
        if is_repl_command(command) {
            println!(
                "{}",
                t(
                    "this command needs the full (daemon) REPL; start without HOTARU_DIRECT to use it",
                    "该命令需要完整(daemon)REPL;不带 HOTARU_DIRECT 启动即可使用"
                )
            );
            continue;
        }
