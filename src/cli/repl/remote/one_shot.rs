...
pub(in crate::cli) async fn try_run_remote_chat(
    ...
    let Some(first) = ipc::receive::<IpcFrame>(&mut stream).await? else {
        bail!("Nonoka core closed the connection before accepting the turn");
    };
    let run_id = match first {
        IpcFrame::Accepted { run_id, .. } => run_id,
        IpcFrame::Error { message, .. } => bail!("{message}"),
        _ => bail!("Nonoka core returned an invalid response"),
    };
    ...
    let completion = loop {
        ...
        let Some(frame) = frame else {
            ...
            bail!("Nonoka core disconnected during the turn");
        };
        ...
        match kind.as_str() {
            ...
            "question.requested" => {
                ...
                notify_if_unfocused(
                    ...
                    t("Nonoka is waiting on you", "Nonoka 在等你回答"),
                    request
                        .questions
                        .first()
    ...
    if config.notifications.on_turn_complete {
        notify_if_unfocused(
            &config,
            focused,
            t("Nonoka finished replying", "Nonoka 回复完成"),
            &result.content,
        );
    }
