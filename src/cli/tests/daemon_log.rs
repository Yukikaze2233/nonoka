...
fn daemon_log_formatter_parses_targets_and_preserves_multiline_content() {
    ...
    assert!(rendered.ends_with("[INFO] [nonoka::qq] listener ready port=8090"));
    assert_eq!(
...
fn daemon_log_formatter_supports_legacy_lines_and_tty_colors() {
    ...
    assert_eq!(parsed.module, "nonoka");
...
fn daemon_log_formatter_colors_entire_active_reply_decisions() {
    ...
    formatter
        .push(
            b"2026-07-29T12:34:56.789Z  INFO nonoka::qq: \xe3\x80\x90\xe7\xbb\xad\xe8\x81\x8a\xe7\xaa\x97\xe5\x8f\xa3\xe5\x88\xa4\xe6\x96\xad\xef\xbc\x9a\xe5\x9b\x9e\xe5\xa4\x8d\xe3\x80\x91\n\xe7\xbb\x93\xe6\x9e\x9c\xef\xbc\x9a\xe5\x9b\x9e\xe5\xa4\x8d\n",
            true,
            &mut reply,
        )
        .unwrap();
    ...
    formatter
        .push(
            b"2026-07-29T12:34:57.789Z  INFO nonoka::qq: \xe3\x80\x90\xe4\xb8\xbb\xe5\x8a\xa8\xe5\x9b\x9e\xe5\xa4\x8d\xe5\x88\xa4\xe6\x96\xad\xef\xbc\x9a\xe4\xb8\x8d\xe5\x9b\x9e\xe5\xa4\x8d\xe3\x80\x91\n\xe7\xbb\x93\xe6\x9e\x9c\xef\xbc\x9a\xe4\xb8\x8d\xe5\x9b\x9e\xe5\xa4\x8d\n",
            true,
            &mut no_reply,
        )
        .unwrap();
    ...
    formatter
        .push(
            b"2026-07-29T12:34:58.789Z  INFO nonoka::qq: listener ready\nplain continuation\n",
            false,
            &mut reset,
        )
        .unwrap();
    let reset = String::from_utf8(reset).unwrap();
    assert!(reset.ends_with("[INFO] [nonoka::qq] listener ready\nplain continuation\n"));
}
...
fn daemon_log_formatter_recognizes_english_active_reply_decisions() {
    ...
    let timestamp = format_daemon_log_line_with_state(
        "2026-07-29T12:34:56.789Z  INFO nonoka::qq: ",
        true,
        &mut color,
    );
...
fn daemon_log_stream_formatter_waits_for_complete_lines() {
    ...
    formatter
        .push(
            b"2026-07-29T12:34:56.789Z  INFO nonoka::qq: part",
            false,
            &mut output,
        )
        .unwrap();
    ...
    assert!(rendered.contains("[INFO] [nonoka::qq] partial\n"));
...
fn recent_daemon_logs_keep_multiline_order_across_rotated_files() {
    ...
    std::fs::write(
        logs_dir.join("nonoka.2026-07-28.log"),
        "2026-07-28T12:00:00Z  INFO nonoka::qq: old event\n  old continuation\n",
    )
    .unwrap();
    std::fs::write(
        logs_dir.join("nonoka.2026-07-29.log"),
        "2026-07-29T12:00:00Z  WARN nonoka::qq: new event\n  new continuation\n判断原因：保持多行\n",
    )
    .unwrap();
    ...
    let rendered = lines
        ...
        .join("\n");
    assert!(!rendered.contains('\x1b'));
    assert!(rendered.contains("[WARN] [nonoka::qq] new event"));
...
fn recent_daemon_logs_include_unstructured_daemon_stream_before_rotating_logs() {
    ...
    std::fs::write(
        logs_dir.join("nonoka.2026-07-29.log"),
        "2026-07-29T12:00:00Z  INFO nonoka::qq: listener ready\n",
    )
    .unwrap();
...
fn daemon_log_follow_cursor_starts_after_the_snapshot_for_each_source() {
    ...
    let rotating = logs_dir.join("nonoka.2026-07-29.log");
...
fn daemon_log_delta_avoids_duplicates_across_append_rotation_and_truncation() {
    ...
    let temp = tempfile::tempdir().unwrap();
    let old = temp.path().join("nonoka.2026-07-28.log");
    let current = temp.path().join("nonoka.2026-07-29.log");
    ...
    std::fs::write(
        &current,
        b"2026-07-29T12:00:00Z  INFO nonoka::qq: first\n  continuation\n",
    )
    .unwrap();
    let mut offset = 0;
    assert!(
    ...
    assert_eq!(rendered.matches("[INFO] [nonoka::qq] first").count(), 1);
    assert_eq!(rendered.matches("  continuation").count(), 1);
    assert_eq!(rendered.matches("[INFO] [nonoka::qq] 群聊").count(), 1);
