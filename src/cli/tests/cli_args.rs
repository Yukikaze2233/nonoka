...
fn variant_is_a_cli_subcommand_with_an_optional_name() {
    let cli = parse_args(["nonoka", "variant"].map(OsString::from).to_vec()).unwrap();
    assert!(matches!(
        cli.command,
        Some(Command::Variant(VariantArgs { name: None }))
    ));

    let cli = parse_args(["nonoka", "variant", "high"].map(OsString::from).to_vec()).unwrap();
    assert!(matches!(
        cli.command,
...
fn web_is_a_cli_subcommand_with_local_server_options() {
    ...
    for arg in ["stop", "status", "restart", "--status", "--stop"] {
        assert!(parse_args(["nonoka", "web", arg].map(OsString::from).to_vec()).is_err());
    }

    let cli = parse_args(["nonoka", "web"].map(OsString::from).to_vec()).unwrap();
    assert!(matches!(
        cli.command,
        Some(Command::Web(WebArgs {
            port: 8300,
    ...
    let cli = parse_args(["nonoka", "web", "-p"].map(OsString::from).to_vec()).unwrap();
    assert!(matches!(
        cli.command,
        Some(Command::Web(WebArgs {
    ...
    for args in [
        vec!["nonoka", "web", "-p", "secret"],
        vec!["nonoka", "web", "--password=secret"],
        vec!["nonoka", "web", "-psecret"],
    ] {
        assert!(parse_args(args.into_iter().map(OsString::from).collect()).is_err());
    }

    let cli = parse_args(
        ["nonoka", "web", "--password-file", "/tmp/nonoka-password"]
            .map(OsString::from)
            .to_vec(),
    )
    .unwrap();
    assert!(matches!(
        ...
        Some(Command::Web(WebArgs {
            ...
            ..
        })) if path == PathBuf::from("/tmp/nonoka-password")
    ));

    assert!(parse_args(["nonoka", "web", "--public"].map(OsString::from).to_vec(),).is_err());
}
...
#[test]
fn explicit_password_file_is_copied_into_private_nonoka_state() {
    let temp = tempfile::tempdir().unwrap();
    let paths = pop_test_paths(temp.path());
...
fn daemon_owns_lifecycle_and_log_commands() {
    for (arg, expected) in [
        ...
        let cli = parse_args(["nonoka", "daemon", arg].map(OsString::from).to_vec()).unwrap();
    ...
    let cli = parse_args(["nonoka", "daemon", "logs"].map(OsString::from).to_vec()).unwrap();
    assert!(matches!(
        cli.command,
        Some(Command::Daemon(DaemonArgs {
...
fn reload_is_a_top_level_command() {
    let cli = parse_args(["nonoka", "reload"].map(OsString::from).to_vec()).unwrap();
    assert!(matches!(cli.command, Some(Command::Reload)));
    assert!(parse_args(["nonoka", "reload", "extra"].map(OsString::from).to_vec()).is_err());
}

#[test]
fn daemon_accepts_a_port_and_defaults_to_start() {
    let cli = parse_args(
        ["nonoka", "daemon", "--port", "9412"]
            .map(OsString::from)
            .to_vec(),
    )
    .unwrap();
    ...
    let cli = parse_args(
        ["nonoka", "daemon", "--port", "9412", "restart"]
            .map(OsString::from)
            .to_vec(),
    )
    .unwrap();
...
fn pop_is_a_cli_subcommand_with_an_optional_count() {
    let cli = parse_args(["nonoka", "pop"].map(OsString::from).to_vec()).unwrap();
    assert!(matches!(
        cli.command,
        Some(Command::Pop(PopArgs { count: None }))
    ));

    let cli = parse_args(["nonoka", "pop", "3"].map(OsString::from).to_vec()).unwrap();
    assert!(matches!(
        cli.command,
        Some(Command::Pop(PopArgs { count: Some(3) }))
    ));
    assert!(parse_args(["nonoka", "pop", "0"].map(OsString::from).to_vec()).is_err());
    assert!(parse_args(["nonoka", "pop", "nope"].map(OsString::from).to_vec()).is_err());
}

#[test]
fn debug_is_a_global_cli_option() {
    for args in [
        &["nonoka", "--debug", "models", "1"][..],
        &["nonoka", "models", "--debug", "1"][..],
        &["nonoka", "hello", "--debug"][..],
        &["nonoka", "ask", "hello", "--debug"][..],
    ] {
        let cli = parse_args(args.iter().map(OsString::from).collect()).unwrap();
        assert!(cli.debug);
    }

    let cli = parse_args(["nonoka", "hello", "--debug"].map(OsString::from).to_vec()).unwrap();
    assert_eq!(cli.message, ["hello"]);

    let cli = parse_args(["nonoka", "--", "--debug"].map(OsString::from).to_vec()).unwrap();
    assert!(!cli.debug);
