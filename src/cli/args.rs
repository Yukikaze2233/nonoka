...
pub enum Command {
    ...
    Tool(ToolArgs),
    /// Internal: run as the Nonoka daemon (spawned by the CLI via
    /// `current_exe`, replacing the former separate `nonokad` binary).
...
/// 工具桥:以本会话身份(NONOKA_SESSION)调用结构化工具。--list 列出的即
