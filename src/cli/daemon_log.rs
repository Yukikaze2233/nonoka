//! daemon 日志的读取、格式化与跟随。
//!
//! `hotaru daemon logs` 要处理几件不太直观的事：日志会轮转（跟随时得跨文件接
//! 上）、可能被截断（要认出来重新定位）、行可能跨多行（结构化日志的多行正文
//! 不能被拆散）、还有 daemon 自己 stdout 的非结构化输出要按时间穿插进去。
//!
//! 格式化那半边负责上色与对齐——`hotaru daemon logs` 是排查问题时最常用的入口，
//! 所以「活跃回复决策」这类关键行整行上色，一眼能扫到。

use crate::cli::*;

pub(in crate::cli) async fn run_daemon_logs(paths: &HotaruPaths, args: DaemonLogsArgs) -> Result<()> {
...
pub(in crate::cli) fn parse_daemon_log_line(line: &str) -> Option<ParsedDaemonLogLine<'_>> {
    ...
    let (module, message) = remainder
        .split_once(": ")
        .filter(|(candidate, _)| is_hotaru_log_target(candidate))
        .unwrap_or(("hotaru", remainder));
...
pub(in crate::cli) fn is_hotaru_log_target(value: &str) -> bool {
    value == "hotaru"
        || value
            .strip_prefix("hotaru::")
...
pub(in crate::cli) fn daemon_log_files(paths: &HotaruPaths) -> Result<Vec<PathBuf>> {
        ...
        Ok(entries) => entries
            ...
            .filter(|path| {
                path.file_name()
                    ...
                    .is_some_and(|name| name.starts_with("hotaru.") && name.ends_with(".log"))
            })
            .collect::<Vec<_>>(),
