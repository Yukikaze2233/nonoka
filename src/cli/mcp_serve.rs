//! `nonoka mcp-serve`:把本会话的工具注册表以 MCP stdio server 形态挂给外部
//! agent(claude-code 供应商中转的工具桥,由 claude 作为子进程拉起)。
//!
//! 与 `nonoka tool-call` 同源:daemon 存活时经 IPC 以 NONOKA_SESSION 的会话身份
    ...
    match method {
        "initialize" => {
            ...
            Ok(serde_json::json!({
                ...
                "capabilities": { "tools": {} },
                "serverInfo": { "name": "nonoka", "version": env!("CARGO_PKG_VERSION") },
            }))
        }
...
async fn list_tools(
    ...
    // 直连回退:与 tool-call 的回退同一构建方式(模式取 NONOKA_TURN_MODE)。
    let config = AppConfig::load_or_default(paths)?;
    let mode = if std::env::var("NONOKA_TURN_MODE").unwrap_or_default() == "dev" {
        AgentMode::Dev
    } else {
        AgentMode::Normal
    };
