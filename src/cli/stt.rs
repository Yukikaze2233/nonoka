//! `nonoka stt`:终端里说一句,识别成文字后当作一条消息发出去,回复照常
//! 流式打印——shellhook/一次性 CLI 的语音形态。
//!
//! 音频来自本机麦克风(daemon 让 `nonoka-voice` 开听写窗),这里只拿文字。
...
/// `nonoka listen`:让前端直接进入等待指令状态(等价于唤醒词命中),
/// 绑到合成器快捷键上就是"按键呼叫"。成功时不输出,便于静默绑定。
pub(in crate::cli) async fn run_listen(paths: &NonokaPaths) -> Result<()> {
    let mut stream = ipc::connect(&paths.ipc_socket())
        .await
        .context(t("Nonoka daemon is not running", "Nonoka daemon 未运行"))?;
...
/// `nonoka voice …`:播报试听、语音会话清空/回看、前端状态。
...
async fn connect_daemon(paths: &NonokaPaths) -> Result<tokio::net::UnixStream> {
...
pub(in crate::cli) async fn dictate_one_sentence(paths: &NonokaPaths) -> Result<Option<String>> {
    let mut stream = ipc::connect(&paths.ipc_socket())
        .await
        .context(t("Nonoka daemon is not running", "Nonoka daemon 未运行"))?;
