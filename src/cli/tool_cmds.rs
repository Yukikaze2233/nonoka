//! `nonoka tool` 与 `nonoka tool-call`：在命令行里直接调工具。
//!
...
pub(in crate::cli) async fn run_tool_call(paths: &NonokaPaths, args: ToolCallArgs) -> Result<()> {
    let config = AppConfig::load_or_default(paths)?;
    let env_mode = std::env::var("NONOKA_TURN_MODE").unwrap_or_default();
    ...
    let Some(name) = args.name.clone() else {
        // 裸 `nonoka tool-call` 是来问路的,给完整帮助而不是一行报错。
        localized_command()
...
mod remote_tool_image_tests {
    ...
    fn ipc_command_response_distinguishes_errors_and_closed_connections() {
        ...
        let rejected = validate_ipc_command_response(Some(IpcFrame::Error {
            code: None,
            message: "Nonoka is busy with another operation".to_string(),
        }))
        .unwrap_err();
