...
pub(in crate::cli) fn localized_command() -> clap::Command {
    ...
    command = command
        .about(t("Nonoka AI assistant", "Nonoka AI 助手"))
        .override_usage(t(
            "nonoka [OPTIONS] [MESSAGE]... [COMMAND]",
            "nonoka [选项] [消息]... [命令]",
        ));
    if is_zh() {
        command = command
            ...
            .help_template("{about}\n\n用法: {usage}\n\n命令:\n{subcommands}\n参数:\n{positionals}\n选项:\n{options}\n{after-help}")
            .after_help("提示：不带参数进入 REPL；直接输入消息会发送一次对话。可在配置界面设置语言，NONOKA_LANG 可临时覆盖。")
            .disable_help_subcommand(true);
    } else {
        command = command
            .after_help(
                "Tip: run without arguments to enter the REPL; pass MESSAGE to send one chat turn. Set the language in the configuration UI; NONOKA_LANG is a temporary override.",
            )
            .disable_help_subcommand(true);
    }
...
pub(in crate::cli) fn root_help_template() -> String {
  ...
  remove-shell-hook  Safely remove installed Nonoka shell hooks
  ...
  bash-init          集成到 bash
  zsh-init           集成到 zsh
  remove-shell-hook  安全删除已安装的 Nonoka shell hook
...
pub(in crate::cli) fn localize_top_args(command: clap::Command) -> clap::Command {
    command
        .mut_arg("debug", |arg| {
            arg.help(t(
                "Write detailed diagnostics to the Nonoka log directory",
                "将详细诊断信息写入 Nonoka 日志目录",
            ))
        })
        .mut_arg("stdout", |arg| {
            arg.help(t(
...
pub(in crate::cli) fn localize_subcommands(mut command: clap::Command) -> clap::Command {
    let descriptions = [
        ...
        (
            "remove-shell-hook",
            "Safely remove installed Nonoka shell hooks",
        ...
        (
            "update-default-kb",
            "Update Nonoka default knowledge base",
            "更新 Nonoka 默认知识库",
        ),
    ...
    // 单独成节(clap 不支持子命令分组);`nonoka <cmd> -h` 不受影响。
    for name in [
        "fish-init",
        "bash-init",
        "zsh-init",
...
pub(in crate::cli) fn localize_export_command(command: clap::Command) -> clap::Command {
    command
        ...
        .mut_arg("index", |arg| {
            arg.help(t(
                "Include the knowledge-base vector index (large; rebuildable with `nonoka kb embed`)",
                "包含知识库向量索引（很大；可用 nonoka kb embed 重建）",
            ))
        })
...
pub(in crate::cli) fn localize_daemon_command(mut command: clap::Command) -> clap::Command {
    let descriptions = [
        (
            "start",
            "Start all configured Nonoka interfaces",
            "启动所有已配置的 Nonoka 接口",
        ),
        (
            "stop",
            "Stop the Nonoka background service",
            "停止 Nonoka 后台服务",
        ),
        (
            "restart",
            "Restart the Nonoka background service",
            "重启 Nonoka 后台服务",
        ),
        (
            "status",
