//! 闹钟工作进程。
//!
//! 闹钟要在 Nonoka 退出之后依然响，所以它是一个 detach 出去的独立进程，只带
...
pub struct AlarmWorkerArgs {
    ...
    #[arg(long)]
    pub time: String,
    #[arg(long, default_value = "Nonoka alarm")]
    pub label: String,
    #[arg(long)]
...
pub(in crate::cli) fn append_alarm_log(paths: &NonokaPaths, line: &str) -> Result<()> {
...
pub(in crate::cli) fn alarm_worker_paths(state_dir: PathBuf, cache_dir: PathBuf) -> NonokaPaths {
    NonokaPaths {
