...
pub(in crate::cli) fn run_pop(paths: &NonokaPaths, args: PopArgs) -> Result<()> {
...
/// single writer.
pub(in crate::cli) async fn run_pop_via_daemon(paths: &NonokaPaths, args: PopArgs) -> Result<()> {
    let state = StateStore::new(paths)?;
        ...
        None => {
            if !io::stdin().is_terminal() || !io::stdout().is_terminal() {
                bail!(
                    "{}",
                    t(
                        "interactive pop requires a terminal; use `nonoka pop <count>`",
                        "交互 pop 需要终端；请使用 `nonoka pop <数量>`",
                    )
                );
            }
...
/// println-based for the direct REPL and one-shot `nonoka pop`).
...
pub(in crate::cli) async fn run_reset(paths: &NonokaPaths) -> Result<()> {
    let config = AppConfig::load_or_default(paths)?;
    let state = StateStore::new(paths)?;
    let memory = MemoryStore::new(&config, paths);
...
/// `nonoka reset-memory`:清空当前人格的长期记忆。daemon 在跑走 IPC,
/// 否则本地直清。
///
/// 不再二次确认:清的只是长期记忆(事实/日记/经历),会话历史、技能和知识库
/// 都不动,和 `/wipe` 那种不可逆的整体抹除不是一个量级。确认弹窗还顺带把
/// 这条命令钉死在终端上——非交互调用只能拿到"需要在终端确认"的报错。
pub(in crate::cli) async fn run_reset_memory_command(paths: &NonokaPaths) -> Result<()> {
...
pub(in crate::cli) fn wipe_summary() -> &'static str {
    t(
        "This erases everything Nonoka has accumulated: memory, every conversation's contents, group-chat contexts, and auto-generated skills. It cannot be undone.",
        "这会抹掉 Nonoka 积累的一切：记忆、所有会话的内容、群聊上下文、自动生成的技能。不可撤销。",
    )
}

pub(in crate::cli) async fn run_wipe(paths: &NonokaPaths, assume_yes: bool) -> Result<()> {
    if !assume_yes {
