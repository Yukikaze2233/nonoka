...
pub fn register(registry: &mut ToolRegistry, config: AppConfig, paths: HotaruPaths) {
...
pub fn register_readonly(registry: &mut ToolRegistry, config: AppConfig, paths: HotaruPaths) {
    registry.register(ToolSpec::new(
...
impl KnowledgeBase {
    pub fn new(config: AppConfig, paths: HotaruPaths) -> Result<Self> {
...
async fn tool_search_readonly(args: Value, config: AppConfig, paths: HotaruPaths) -> Result<String> {
...
async fn tool_find_readonly(args: Value, config: AppConfig, paths: HotaruPaths) -> Result<String> {
    ensure_enabled(&config)?;
...
async fn tool_read_readonly(args: Value, config: AppConfig, paths: HotaruPaths) -> Result<String> {
    ensure_enabled(&config)?;
    let name = args
        .get("file_name")
        .and_then(Value::as_str)
...
async fn tool_upload(args: Value, config: AppConfig, paths: HotaruPaths) -> Result<String> {
    ensure_enabled(&config)?;
    if !config.plugins.knowledge_base.upload_tool_enabled {
...
async fn tool_edit(args: Value, config: AppConfig, paths: HotaruPaths) -> Result<String> {
    ensure_enabled(&config)?;
    let name = args
        .get("file_name")
        .and_then(Value::as_str)
    ...
    Ok(json!({
        ...
        "warning": if name.starts_with("default-kb/") { Some("default-kb files may be overwritten by hotaru update-default-kb") } else { None::<&str> },
    })
    .to_string())
}

async fn tool_remove(args: Value, config: AppConfig, paths: HotaruPaths) -> Result<String> {
    ensure_enabled(&config)?;
    let name = args
        .get("file_name")
        .and_then(Value::as_str)
    ...
    Ok(json!({
        "ok": true,
        "path": rel,
        "warning": if name.starts_with("default-kb/") { Some("default-kb files may be restored by hotaru update-default-kb") } else { None::<&str> },
    })
    .to_string())
}
...
mod tests {
    ...
    pub(super) fn test_paths(root: &Path) -> HotaruPaths {
        HotaruPaths {
            ...
            fish_hook_file: root.join("fish/conf.d/hotaru.fish"),
