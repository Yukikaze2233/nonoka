...
/// ——路径和损坏容错的规则只该有一份。
pub(crate) fn session_todos(paths: &NonokaPaths, session: &str) -> Vec<Todo> {
...
pub(crate) fn clear_session_todos(paths: &NonokaPaths, session: &str) -> Result<()> {
...
fn load_todos(paths: &NonokaPaths, session: &str) -> Vec<Todo> {
...
fn save_todos(paths: &NonokaPaths, session: &str, todos: &[Todo]) -> Result<()> {
