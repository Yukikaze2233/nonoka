...
mod tests {
    ...
    async fn effective_workdir_returns_scoped_workspace() {
        let workspace = PathBuf::from("/tmp/nonoka-turn-workspace");
    ...
    async fn workspace_visible_inside_select_nested_future() {
        let workspace = PathBuf::from("/tmp/nonoka-select-workspace");
...
/// 本回合的发起来源(dsh goal 权限模型的 Nonoka 化:不扫会话事件,发起方
/// 在起回合时如实声明)。
...
tokio::task_local! {
    /// 工具桥递归深度:回合内 run_command 起的脚本经 `nonoka tool-call` 打回
