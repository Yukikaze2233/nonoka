...
pub fn register(registry: &mut ToolRegistry, history_file: PathBuf, config: crate::config::AppConfig) {
    registry.register(
        ToolSpec::new(
            "query_token_usage",
            "Query Hotaru's token usage statistics: totals, request count, cache hit rate, and the per-source (agent / messaging platforms) model breakdown. range: 1d (rolling 24h, default) / 7d / 30d / all.",
            json!({
                "type": "object",
                "properties": {
                    "range": {
