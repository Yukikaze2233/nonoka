...
fn stable_catalog(config: &AppConfig, paths: &HotaruPaths) -> Result<(Vec<SkillEntry>, [u8; 32])> {
    for _ in 0..3 {
...
fn register_load_skill(
    ...
    let description = format!(
        ...
        "The skill name must match one of the available skills listed below. Use this tool before applying a skill or using any scripts/resources from that skill. Skill allowed-tools metadata never grants Hotaru permissions.",
...
fn load_skill(args: Value, config: &AppConfig, paths: &HotaruPaths) -> Result<String> {
...
fn create_skill(args: Value, config: &AppConfig, paths: &HotaruPaths) -> Result<String> {
    let name = required_string(&args, "name")?;
...
fn update_skill(args: Value, config: &AppConfig, paths: &HotaruPaths) -> Result<String> {
    let name = required_string(&args, "name")?;
...
fn publish_skill(args: Value, paths: &HotaruPaths) -> Result<String> {
...
fn delete_skill(args: Value, config: &AppConfig, paths: &HotaruPaths) -> Result<String> {
    let name = required_string(&args, "name")?;
    let scope_value = required_string(&args, "scope")?;
    let scope = SkillScope::parse(Some(&scope_value))?;
...
mod tests {
    ...
    fn loaded_skill_exposes_standard_metadata_without_granting_permissions() {
        ...
        assert!(loaded.contains("<compatibility>Hotaru</compatibility>"));
        assert!(loaded
