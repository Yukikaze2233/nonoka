...
pub(crate) async fn add_meme(args: Value, config: &AppConfig, paths: &NanokaPaths) -> Result<String> {
...
pub(crate) async fn update_meme(args: Value, config: &AppConfig, paths: &NanokaPaths) -> Result<String> {
    let library = selected_library(&args, config);
    let library_lock = library_lock(&library);
    let _guard = library_lock.lock().await;
...
pub(crate) async fn delete_meme(args: Value, config: &AppConfig, paths: &NanokaPaths) -> Result<String> {
    let library = selected_library(&args, config);
    let library_lock = library_lock(&library);
    let _guard = library_lock.lock().await;
