...
pub(crate) const BUILTIN_MEMES_DIR: &str = "/usr/share/hotaru/memes";
...
pub(crate) fn load_library(paths: &HotaruPaths, library: &str) -> Result<Vec<LoadedMeme>> {
...
pub(crate) fn find_meme(paths: &HotaruPaths, library: &str, id: &str) -> Result<Option<LoadedMeme>> {
...
pub(crate) fn meme_ref_exists(paths: &HotaruPaths, meme: &MemeRef) -> Result<bool> {
...
pub(crate) fn builtin_library_dir(library: &str) -> PathBuf {
    if let Some(path) = std::env::var_os("HOTARU_MEMES_DIR") {
...
pub(crate) fn user_library_dir(paths: &HotaruPaths, library: &str) -> PathBuf {
