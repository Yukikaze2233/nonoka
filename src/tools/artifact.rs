...
pub fn register_webui(registry: &mut ToolRegistry, paths: &NanokaPaths, session_id: &str) {
...
pub fn managed_manifest(paths: &NanokaPaths, session_id: &str) -> Result<String> {
    let root = paths.data_dir.join("artifacts");
...
fn register_create(registry: &mut ToolRegistry, root: PathBuf, session_id: &str) {
    ...
    registry.register(ToolSpec::new_with_progress(
        "create_artifact",
        "Create or update a completed deliverable in Nanoka's managed Artifact workspace and display it in the WebUI. Use this for reports, documents, standalone code, HTML, JSON, CSV, and other files the user should inspect. Do not use it for routine project source edits.",
        json!({
            "type": "object",
            "properties": {
                "filename": {
