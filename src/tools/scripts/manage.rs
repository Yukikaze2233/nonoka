...
pub(crate) fn user_layers(config: &AppConfig, paths: &NonokaPaths) -> [ScriptLayer; 2] {
    [
        ScriptLayer {
            scope: "persona",
...
fn builtin_has_script(config: &AppConfig, paths: &NonokaPaths, id: &str) -> Result<bool> {
...
pub(crate) fn list_scripts_handler(config: &AppConfig, paths: &NonokaPaths) -> Result<String> {
    let roots = script_scan_roots(config, paths);
