...
pub fn clear_aur_review_state(paths: &NanokaPaths) -> anyhow::Result<()> {
...
pub fn builtin_registry(config: &AppConfig, paths: &NanokaPaths) -> ToolRegistry {
...
pub fn webui_artifact_manifest(paths: &NanokaPaths, session_id: &str) -> anyhow::Result<String> {
...
pub fn dev_registry(config: &AppConfig, paths: &NanokaPaths) -> ToolRegistry {
    let mut registry = ToolRegistry::new();
    registry.set_default_timeout_secs(config.tools.default_timeout_secs);
    install_builtin_guards(&mut registry, config);
...
pub fn restricted_platform_registry(config: &AppConfig, paths: &NanokaPaths) -> ToolRegistry {
    let mut registry = ToolRegistry::new();
    registry.set_default_timeout_secs(config.tools.default_timeout_secs);
    web::register_fetch(&mut registry);
    weather::register(&mut registry);
...
mod tests {
    ...
    pub(super) fn test_paths(root: &std::path::Path) -> NanokaPaths {
        NanokaPaths {
            ...
            fish_hook_file: root.join("config/fish/conf.d/nanoka.fish"),
