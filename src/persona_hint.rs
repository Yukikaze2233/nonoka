...
/// 默认 Nonoka 人格的 scope 是 "default",对应 hints/default.md。
pub fn manual_hint_path(config: &AppConfig, paths: &NonokaPaths, scope: &str) -> PathBuf {
    config
...
/// 默认 Nonoka 人格是否生效:未选自定义人格、也没有任何自定义 system
/// prompt 时,base_system_prompt 落到内置 nonoka.md——只有这时内置的
/// 提示/预设对话默认值才有意义。
fn uses_default_persona(config: &AppConfig, paths: &NonokaPaths) -> bool {
    config
...
/// 读取手写提醒。文件缺失且默认 Nonoka 人格生效时回退到内置默认提示;
/// 文件存在但为空 = 显式停用手写(回到自动蒸馏)。
pub fn manual_hint(config: &AppConfig, paths: &NonokaPaths, scope: &str) -> Option<String> {
    match std::fs::read_to_string(manual_hint_path(config, paths, scope)) {
        ...
        Err(_) if scope == "default" && uses_default_persona(config, paths) => {
            let text = crate::prompts::default_nonoka_hint();
            let text = text.trim();
            (!text.is_empty()).then(|| text.to_string())
        }
        Err(_) => None,
...
pub fn dialogs_path(config: &AppConfig, paths: &NonokaPaths, scope: &str) -> PathBuf {
    config
        .prompts_dir_path(paths)
        .join("dialogs")
        .join(format!("{scope}.md"))
...
/// 预设对话原文(注入与 TUI 表单预填共用)。文件缺失且默认 Nonoka 人格
/// 生效时回退到内置默认;文件存在但为空 = 显式停用。
pub fn dialogs_raw(config: &AppConfig, paths: &NonokaPaths, scope: &str) -> String {
    match std::fs::read_to_string(dialogs_path(config, paths, scope)) {
        Ok(text) => text.trim().to_string(),
        Err(_) if scope == "default" && uses_default_persona(config, paths) => {
            crate::prompts::default_nonoka_dialogs().trim().to_string()
        }
        Err(_) => String::new(),
    }
}
...
/// 解析后的预设对话对(注入用)。
pub fn load_dialogs(config: &AppConfig, paths: &NonokaPaths, scope: &str) -> Vec<(String, String)> {
...
/// Nonoka 表单预填:default scope 的文件现值,缺失时直接给内置默认——
/// 不看当前激活的是哪个人格,因为表单编辑的就是 Nonoka 本体的附属文件。
pub fn nonoka_aux_prefill(config: &AppConfig, paths: &NonokaPaths) -> (String, String) {
    let hint = match std::fs::read_to_string(manual_hint_path(config, paths, "default")) {
        Ok(text) => text.trim().to_string(),
        Err(_) => crate::prompts::default_nonoka_hint().trim().to_string(),
    };
    let dialogs = match std::fs::read_to_string(dialogs_path(config, paths, "default")) {
        Ok(text) => text.trim().to_string(),
        Err(_) => crate::prompts::default_nonoka_dialogs().trim().to_string(),
    };
    (hint, dialogs)
}
...
fn auto_cache_dir(paths: &NonokaPaths) -> PathBuf {
...
mod tests {
    ...
    fn manual_hint_precedence_file_over_embedded_over_none() {
        ...
        assert_eq!(
            ...
            Some(crate::prompts::default_nonoka_hint().trim())
        );
        // 自定义 scope 不回退内置。
