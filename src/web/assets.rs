...
pub(in crate::web) async fn index_asset(headers: HeaderMap) -> Response {
    ...
    static VERSIONED_INDEX: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
        let mut html = INDEX_HTML
            ...
            .replace(
                "src=\"/lightbox.js\"",
                concat!("src=\"/lightbox.js?v=", env!("NONOKA_BUILD_ID"), "\""),
            )
            .replace(
                "src=\"/todos.js\"",
                concat!("src=\"/todos.js?v=", env!("NONOKA_BUILD_ID"), "\""),
            )
            .replace(
                "src=\"/shared.js\"",
                concat!("src=\"/shared.js?v=", env!("NONOKA_BUILD_ID"), "\""),
            )
            .replace(
        ...
        for (name, _) in DASH_SCRIPTS {
            html = html.replace(
                ...
                &format!("src=\"/dash/{name}?v={}\"", env!("NONOKA_BUILD_ID")),
            );
        }
        html
    });
...
pub(in crate::web) async fn logo_asset(headers: HeaderMap) -> Response {
    embedded_asset(&headers, NONOKA_LOGO, "image/png")
}
...
pub(in crate::web) async fn wallpaper_asset(headers: HeaderMap) -> Response {
    embedded_asset(&headers, NONOKA_WALLPAPER, "image/png")
}
