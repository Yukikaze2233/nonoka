...
//! - `GET /api/voice/devices`:麦克风列表(问 `nonoka-voice devices`);
//! - `GET /api/voice/stream`(WebSocket):流式听写。浏览器把 16kHz 单声道
//!   PCM16 LE 以二进制帧持续推上来,daemon 转给 `nonoka-voice` 做 VAD/分句/
...
pub(in crate::web) async fn voice_devices(
    ...
    let Some(binary) = voice_bridge::locate_binary() else {
        return Ok(json_devices(Vec::new(), Some("nonoka-voice not installed")));
    };
