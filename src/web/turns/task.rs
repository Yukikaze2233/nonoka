    ...
    let setup = (|| -> Result<(Agent, AgentTurnControl)> {
        ...
        if local_webui && mode == AgentMode::Normal {
            ...
            turn_system_context.push(format!(
                "<artifact-workspace>\n{manifest}\nUse read_artifact and apply_artifact_patch with bare artifact file names to work on existing artifacts; do not glob the managed directory or guess ~/.nanoka paths.\n</artifact-workspace>"
            ));
            runtime_system_context.push(
                "<artifact-policy>\n\
                You are working in the Nanoka WebUI and have artifact presentation tools.\n\
