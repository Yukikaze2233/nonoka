...
async fn platform_session_reset_is_serialized_per_target_session() {
    ...
    let target = state
        .state_store
        .create_session("nanoka", "qq target", "user", None)
        .unwrap();
    let other = state
        .state_store
        .create_session("nanoka", "other", "user", None)
        .unwrap();
...
data = b""
try:
    while b"NANOKA-E2E-END" not in data:
...
"#;
    ...
    // 走生产写线程:Write 分片 + Finish(flush + SIGWINCH),与流式回写同路。
    {
        ...
        ops_tx
            .send(TtyWriteOp::Write(
                "\x1b[1m✦ Nanoka 后台任务跟进\x1b[0m\r\n".to_string(),
            ))
            .unwrap();
        let mut body = String::new();
        push_rendered_line(
            "**粗体** 与 `代码` NANOKA-E2E-END",
    ...
    assert!(
        text.contains("Nanoka 后台任务跟进"),
        "master 端应读到标题,实际: {text:?}"
    );
    assert!(text.contains("NANOKA-E2E-END"), "正文应完整到达");
