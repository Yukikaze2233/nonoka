...
fn startup_repairs_a_platform_owned_current_session() {
    ...
    let qq_session = store
        .create_session("nanoka", "QQ group 20000", "user", None)
        .unwrap();
    store
        .bind_platform_session(
            &PlatformSessionBindingKey {
    ...
    ensure_local_current_session(&store, "nanoka").unwrap();
...
fn persona_identity_uses_default_and_custom_values() {
    ...
    assert_eq!(default.name, "Nanoka");
    assert_eq!(default.avatar_url.as_deref(), Some("/assets/nanoka-logo.png"));
