...
//! 独立的 `nonoka-voice` 进程里(见 `voice::worker` 的信令表)。这里负责:
//!
//! - 定位并拉起/看护 `nonoka-voice`(崩溃退避重启,daemon 关闭时收走,
//!   配置重载时按需重启);
//! - 持有它的 `VoiceAttach` 信令连接;
//! - 唤醒路:`voice.command` → 在「语音会话」lane 起回合 → 完成后桌面通知
//!   回复摘要 + 提示音;进行中再开口即取消(打断);
//! - 听写中继:REPL `/stt`、`nonoka stt` 认领听写流(本机麦),识别文本流回;
...
pub(crate) const BINARY_NAME: &str = "nonoka-voice";
...
/// 找 `nonoka-voice`:先看主程序同目录,再扫 PATH。
pub(crate) fn locate_binary() -> Option<PathBuf> {
    if let Ok(exe) = crate::paths::nonoka_executable() {
    ...
    match kind {
        ...
        "voice.wake" => {
            ...
            if cfg!(target_os = "linux") {
                ...
                notify(
                    state,
                    t("Nonoka is listening", "Nonoka 在听"),
                    t("speak now", "请讲"),
                );
            } else {
        ...
        "voice.command" => {
            ...
            cancel_active_run(state);
            notify(state, t("Nonoka heard", "Nonoka 收到"), &clip(&text, 80));
            let state = state.clone();
            tokio::spawn(async move {
        ...
        "voice.window_closed" | "voice.timeout" => {
            ...
            if text_field(&data, "reason") == "listen" {
                ...
                notify(state, "Nonoka", t("stopped listening", "不听了"));
            }
        }
        "voice.transcribed" => {
        ...
        "voice.error" => {
            ...
            notify(
                state,
                t("Nonoka voice stopped", "语音服务已停止"),
                &clip(&message, 120),
            );
        }
...
/// VoiceSpeak 处理:`nonoka voice say` / 设置页试听。
...
async fn wait_attached(state: &DaemonState, timeout_ms: u64) -> Result<(), &'static str> {
    ...
    if locate_binary().is_none() {
        return Err("找不到 nonoka-voice 可执行文件,请安装语音组件");
    }
    ensure_worker(state);
    let mut waited = 0u64;
