...
pub async fn run(paths: NonokaPaths, args: WebArgs) -> Result<()> {
    ...
    let listener = match tokio::net::TcpListener::bind(SocketAddr::new(bind_ip, args.port)).await {
                ...
                && error.kind() == std::io::ErrorKind::AddrInUse =>
        {
            tracing::warn!(
                ...
                "{}",
                t(
                    "Nonoka WebUI default port is occupied; selecting an ephemeral port",
                    "Nonoka WebUI 默认端口已被占用；将选择临时端口"
                )
            );
            tokio::net::TcpListener::bind(SocketAddr::new(bind_ip, 0))
                .await
                .context("binding Nonoka WebUI to an ephemeral fallback port")?
        }
        Err(error) => {
            return Err(error)
                .with_context(|| format!("binding Nonoka WebUI to {bind_ip}:{}", args.port));
        }
    };
    ...
    drop(ipc_lease);
    serve_result.context("serving Nonoka WebUI")?;
    actor_result
}
...
pub(in crate::web) async fn follow_run(
    ...
    loop {
        ...
        if record.kind == "resync_required" {
            ipc::send(
                stream,
                &IpcFrame::error("Nonoka core event history was exhausted"),
            )
            .await?;
            break;
        }
...
pub(in crate::web) fn router(state: DaemonState) -> Router {
    Router::new()
        ...
        .route("/assets/nonoka-logo.png", get(logo_asset))
        .route("/assets/nonokawallpaper.png", get(wallpaper_asset))
        ...
        // Backward-compatible endpoint used by earlier Nonoka releases.
        .route(
            "/onebot/v11/ws",
...
pub(in crate::web) fn build_etag() -> &'static HeaderValue {
    static ETAG_VALUE: std::sync::LazyLock<HeaderValue> = std::sync::LazyLock::new(|| {
        HeaderValue::from_str(concat!("\"", env!("NONOKA_BUILD_ID"), "\""))
