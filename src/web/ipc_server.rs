...
pub(in crate::web) fn start_ipc_server(
    ...
    let lease = ipc::acquire_web_core(&state.paths)
        .context("another Nonoka core is already running or starting")?;
    let socket_path = state.paths.ipc_socket();
    let listener = tokio::net::UnixListener::bind(&socket_path)
        .with_context(|| format!("binding Nonoka IPC socket at {}", socket_path.display()))?;
            ...
            let (stream, _) = match listener.accept().await {
                ...
                Err(error) => {
                    tracing::warn!(
                        error = %error,
                        "{}",
                        t("Nonoka IPC listener stopped", "Nonoka IPC 监听器已停止")
                    );
                    break;
                }
            };
            ...
            tokio::spawn(async move {
                ...
                if let Err(error) = handle_ipc_connection(connection_state, stream).await {
                    tracing::debug!(
                        ...
                        "{}",
                        t(
                            "Nonoka IPC connection closed with an error",
                            "Nonoka IPC 连接因错误关闭"
                        )
                    );
                }
...
pub(in crate::web) async fn handle_ipc_connection(
    ...
    .await
    .context("timed out waiting for a Nonoka IPC request")??
    else {
        return Ok(());
    };
    ...
    match request.command {
        ...
        IpcCommand::GetReplSession { mode } => {
            ...
            // (shellhook)的车道。于是第一次 `nonoka normal` 就把 REPL 焊在终端
        ...
        IpcCommand::ReloadConfig => {
            ...
            match receiver.await {
                ...
                Err(_) => {
                    ...
                    ipc::send(
                        &mut stream,
                        &IpcFrame::error("Nonoka core stopped while reloading configuration"),
                    )
                    .await?
                }
            }
        ...
        IpcCommand::ResetConversation { target } => {
            ...
            {
                release_admin(&state.manager);
                anyhow::bail!("Nonoka core worker is unavailable");
            }
            match receiver.await {
                Ok(Ok(())) => {
                    ipc::send(
                ...
                Err(_) => {
                    release_admin(&state.manager);
                    anyhow::bail!("Nonoka core stopped while resetting the conversation");
                }
            }
        }
        ...
        IpcCommand::Undo { target } => {
            ...
            match receiver.await {
                ...
                Err(_) => {
                    release_admin(&state.manager);
                    anyhow::bail!("Nonoka core stopped while undoing the conversation");
                }
            }
        }
        IpcCommand::Pop { target, turn_ids } => {
            ...
            match receiver.await {
                ...
                Err(_) => {
                    release_admin(&state.manager);
                    anyhow::bail!("Nonoka core stopped while popping the conversation");
                }
            }
        }
        IpcCommand::Compact { target } => {
            ...
            match receiver.await {
                ...
                Err(_) => {
                    release_admin(&state.manager);
                    anyhow::bail!("Nonoka core stopped while compacting the conversation");
                }
            }
        }
...
pub(in crate::web) async fn switch_session_via_actor(
    ...
    {
        release_admin(&state.manager);
        return Err("Nonoka core worker is unavailable".to_string());
    }
    match receiver.await {
        Ok(Ok(())) => Ok(()),
        ...
        Err(_) => {
            release_admin(&state.manager);
            Err("Nonoka core stopped while switching sessions".to_string())
        }
    }
}
...
pub(in crate::web) async fn switch_session_via_actor_reserved(
    ...
    match receiver.await {
        Ok(Ok(())) => Ok(()),
        Ok(Err(AdminFailure::Invalid(message) | AdminFailure::Internal(message))) => Err(message),
        Err(_) => Err("Nonoka core stopped while switching sessions".to_string()),
    }
}
