...
//! (`mimo-v2.5-tts` 系列,返回 base64 的 wav);播放在 nonoka-voice 里。
...
mod tests {
    ...
    fn mimo_request_body_follows_docs() {
        ...
        let dir = std::env::temp_dir().join(format!("nonoka-mimo-{}", std::process::id()));
    ...
    /// 真实 MiMo 接口(要 `MIMO_API_KEY`):合成一句写到 `NONOKA_MIMO_OUT`(缺省
    ...
    async fn synthesize_mimo_real_api() {
        ...
        if let Ok(voice) = std::env::var("NONOKA_MIMO_VOICE") {
            cfg.voice = voice;
        }
        ...
        let audio = synthesize_mimo(&cfg, "今天也是充满希望的一天")
            .await
            .unwrap();
        let out = std::env::var("NONOKA_MIMO_OUT").unwrap_or_else(|_| "mimo-test.wav".to_string());
