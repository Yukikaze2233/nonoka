...
impl GroupManagementPlugin {
    ...
    fn register(
        ...
        if context.conversation.kind != ConversationKind::Group {
            // 群聊之外只给 Hotaru 管理员留跨群查询入口（group_id 必填）
    ...
    fn register_history_query(
        ...
        registry.register(ToolSpec::new(
            ...
            "Query QQ group management records (mute/kick/title). view=events lists individual actions newest-first; view=stats aggregates per member (ban_count, kick_count, total mute duration). Hotaru admins may pass group_id to query another group; group_id is required outside that group's chat.",
