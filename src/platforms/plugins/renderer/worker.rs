...
pub(in crate::platforms::plugins::renderer) const WORKER_ENV: &str = "NANOKA_INTERNAL_RENDERER_WORKER";
        ...
        let mut child = command.spawn().with_context(|| {
            format!(
                "starting the long-image renderer worker ({}); \
                 if Nanoka was upgraded or rebuilt while running, restart the daemon",
