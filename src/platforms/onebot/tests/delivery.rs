...
async fn file_upload_falls_back_to_base64_after_url_failure() {
    ...
    let (handle, mut frames) = test_connection(Some("http://nonoka.test:8300".to_string()));
    ...
    assert!(first["params"]["file"]
        .as_str()
        .unwrap()
        .starts_with("http://nonoka.test:8300/api/platform-assets/"));
    route_api_response(
        &handle,
        json!({
            "status": "failed",
