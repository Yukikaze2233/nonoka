...
//!
//! OneBot 是**反向连接**：客户端主动连过来，Nonoka 是服务端。所以要有注册表管理
    ...
    pub(crate) fn commit(mut self) {
            ...
            if inner.active_port == self.desired_port {
                ...
                inner.task = self.listener.take().map(|listener| {
                    ...
                    tokio::spawn(async move {
                        ...
                        .await
                        {
                            tracing::error!(target: "nonoka::qq", error = %error, "{}", t("Tencent QQ listener stopped", "腾讯 QQ 监听器已停止"));
                        }
                    })
                });
                previous
        ...
        if previous_port != self.desired_port {
            match self.desired_port {
                Some(port) => {
                    tracing::info!(target: "nonoka::qq", port, path = "/ws", "{}", t("Tencent QQ listener ready", "腾讯 QQ 监听器已就绪"))
                }
                None => {
                    tracing::info!(target: "nonoka::qq", "{}", t("Tencent QQ listener disabled", "腾讯 QQ 监听器已禁用"))
                }
            }
        }
    }
...
pub(crate) async fn onebot_ws(
    ...
    if !connection_authorized(&headers, &config.access_token, peer) {
        if config.access_token.trim().is_empty() {
            tracing::warn!(target: "nonoka::qq", %peer, reason = "non_loopback_without_token", "{}", t("OneBot client rejected", "OneBot 客户端已拒绝"));
        } else {
            tracing::warn!(target: "nonoka::qq", %peer, reason = "bad_token", "{}", t("OneBot client rejected", "OneBot 客户端已拒绝"));
        }
...
pub(in crate::platforms::onebot) async fn connection_loop(
    ...
    tracing::info!(target: "nonoka::qq", self_id, generation, "{}", t("OneBot client connected", "OneBot 客户端已连接"));
