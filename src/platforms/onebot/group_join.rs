...
pub(in crate::platforms::onebot) async fn handle_friend_add_request(state: DaemonState, conn: ConnectionHandle, event: Value) {
    ...
    let Some(flag) = flag else {
        tracing::warn!(target: "nonoka::qq", "{}", t("OneBot friend request is missing flag", "OneBot 好友请求缺少 flag"));
        return;
    };
    if self_id == 0 || user_id == 0 {
        tracing::warn!(target: "nonoka::qq", self_id, user_id, "{}", t("OneBot friend request has invalid ids", "OneBot 好友请求包含无效 QQ 号"));
        return;
    }
