        ...
        let delivered = match self.adapter.send(primary.clone()).await {
            ...
            Err(error) => {
                ...
                match (partially_delivered, prepared.fallback) {
                    ...
                    (false, Some(fallback)) => {
                        tracing::warn!(target: "nonoka::qq", error = %error, "{}", crate::i18n::text("transformed platform message failed; sending fallback", "转换后的平台消息发送失败；正在发送回退消息"));
        ...
        for message in prepared.after_success {
            ...
            match self.adapter.send(message).await {
                ...
                Err(error) => {
                    ...
                    tracing::warn!(target: "nonoka::qq", error = %error, "{}", crate::i18n::text("platform plugin follow-up send failed", "平台插件后续消息发送失败"));
                }
            }
        }
