...
pub(crate) fn register(registry: &mut ToolRegistry, context: Arc<PlatformTurnContext>) {
    ...
    let parameters = if host_tools_allowed {
        ...
        // 非管理员触发也能发 Hotaru 自产的生成图(执行侧按生图目录校验豁免);
...
fn register_mention(registry: &mut ToolRegistry, context: Arc<PlatformTurnContext>) {
    registry.register(
        ToolSpec::new(
            "qq_mention_users",
            "Make Hotaru's next outgoing message in the current QQ group natively @ one or more members. Provide exact QQ IDs; use get_group_members_info first when only names are known. These explicit targets replace the automatic reply mention but preserve its message-quote behavior. This tool does not send a separate message.",
            json!({
                "type": "object",
                "properties": {
                    "user_ids": {
...
async fn send(arguments: Value, context: Arc<PlatformTurnContext>) -> Result<String> {
    ...
    if (!images.is_empty() || !files.is_empty()) && !context.host_tools_allowed() {
        // 附件门槛防的是把宿主上任意文件发给陌生人;Hotaru 自己刚生成的图不在
