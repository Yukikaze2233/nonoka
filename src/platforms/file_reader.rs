...
pub(crate) fn register(
        ...
        == crate::platform_types::ConversationKind::Group
    {
        ...
        "Read a text file uploaded through the current QQ/platform conversation. `file` is either a file id from the visible chat history (e.g. file_<message_id>_1) or an absolute path Hotaru already downloaded under its platform_files cache. Compressed archives, executables, images, video, and other binary formats are rejected; text is capped at 128 KiB per call."
    };
    registry.register(
        ToolSpec::new(
            "read_platform_file",
...
fn read_platform_text(path: &Path, name: &str, size: u64) -> Result<String> {
    ...
    if binary_extension {
        bail!("`{name}` is a binary or compressed format; Hotaru cannot read it as text");
    }
