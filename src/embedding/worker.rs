...
pub(crate) const WORKER_ENV: &str = "NONOKA_INTERNAL_EMBEDDING_WORKER";
pub(crate) const WORKER_ARG: &str = "__embedding-worker";
const MODEL_DIR_ENV: &str = "NONOKA_EMBEDDING_WORKER_MODEL_DIR";
const RUNTIME_LIB_WORKER_ENV: &str = "NONOKA_EMBEDDING_WORKER_RUNTIME_LIB";
const IDLE_SECS_ENV: &str = "NONOKA_EMBEDDING_WORKER_IDLE_SECS";
...
async fn spawn_worker(
    ...
    let mut child = command.spawn().with_context(|| {
        format!(
            "starting the embedding worker ({}); if Nonoka was upgraded or rebuilt while running, restart the daemon",
            executable.display()
        )
    })?;
    let stdin = child
