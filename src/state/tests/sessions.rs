...
fn session_crud_switching_and_persona_adoption() {
    ...
    let listed = store.list_sessions("hotaru").unwrap();
    ...
    assert!(store
        .find_session_by_name("hotaru", "旅行计划")
        .unwrap()
        .is_some());
    ...
    assert_eq!(store.list_sessions("hotaru").unwrap().len(), 1);
...
fn persona_reset_clears_active_local_and_onebot_contexts_only() {
    ...
    let local = store.create_session("hotaru", "local", "user", None).unwrap();
    let second = store
        .create_session("hotaru", "second", "user", None)
        .unwrap();
    let other_persona = store
        .create_session("other", "other", "user", None)
        .unwrap();
    let qq = store.create_session("hotaru", "qq", "user", None).unwrap();
    store
        ...
        .unwrap();
    let subagent = store
        .create_session("hotaru", "child", "subagent", Some(&local.session_id))
        .unwrap();
    let second_child = store
        .create_session("hotaru", "second-child", "subagent", Some(&second.session_id))
        .unwrap();
    ...
    let targets = store.persona_reset_session_ids("hotaru", "onebot").unwrap();
    ...
    let cleared = store.reset_persona_contexts("hotaru", "onebot").unwrap();
    ...
    assert_eq!(
        store.platform_session_bindings("hotaru", "onebot").unwrap()[0].session_id,
        qq.session_id
    );
}
...
fn local_session_listing_excludes_platform_owned_history() {
    let (_temp, store) = test_store();
    let local = store
        .create_session("hotaru", "shared name", "user", None)
        .unwrap();
    let platform = store
        .create_session("hotaru", "shared name", "user", None)
        .unwrap();
    let key = platform_binding_key("20000", None, "hotaru");
    store
        .bind_platform_session(&key, &platform.session_id)
        .unwrap();

    let all_ids = store
        .list_sessions("hotaru")
        .unwrap()
        .into_iter()
    ...
    assert_eq!(
        store
            .find_local_session_by_name("hotaru", "SHARED NAME")
            .unwrap()
            .unwrap()
            .session_id,
        local.session_id
...
fn wiping_the_persona_takes_the_subagent_rows_with_it() {
    ...
    let audit = store
        .create_session("hotaru", "深挖", "subagent", Some(&parent))
        .unwrap();
    store
    ...
    store.reset_persona_contexts("hotaru", "onebot").unwrap();
    assert_eq!(
...
fn a_subagent_run_recorded_before_the_cache_column_stays_out_of_the_rate() {
    ...
    let parent = store.session_id();
    let audit = store
        .create_session("hotaru", "升级前的一次", "subagent", Some(&parent))
        .unwrap();
...
fn an_estimated_subagent_run_never_reaches_the_cache_denominator() {
    ...
    let parent = store.session_id();
    let audit = store
        .create_session("hotaru", "估算的一次", "subagent", Some(&parent))
        .unwrap();
...
fn subagent_audit_sessions_are_hidden_and_expire() {
    ...
    let parent = store.session_id();
    let audit = store
        .create_session("hotaru", "探索代码库", "subagent", Some(&parent))
        .unwrap();
...
fn one_shot_sessions_stay_invisible_and_stale_ones_are_swept() {
    ...
    let user = store
        .create_session("hotaru", "real", USER_SESSION_KIND, None)
        .unwrap();
    let ask = store
        .create_session("hotaru", "一次性对话", ASK_SESSION_KIND, None)
        .unwrap();
    ...
    assert!(store
        .find_local_session_by_name("hotaru", "一次性对话")
        .unwrap()
        .is_none());
...
fn repl_session_pointer_is_separate_and_drops_when_stale() {
    ...
    let repl = store
        .create_session("hotaru", "repl lane", USER_SESSION_KIND, None)
        .unwrap();

    assert!(store.repl_session("hotaru").unwrap().is_none());
    store.set_repl_session("hotaru", &repl.session_id).unwrap();
    assert_eq!(
        store.repl_session("hotaru").unwrap().as_deref(),
...
fn clearing_pinned_session_content_is_isolated_and_preserves_usage_and_binding() {
    ...
    let target_record = store
        .create_session("hotaru", "qq:10000:private:42", "user", None)
        .unwrap();
    ...
    target
        ...
        .unwrap();
    let binding = platform_binding_key("42", None, "hotaru");
    store
