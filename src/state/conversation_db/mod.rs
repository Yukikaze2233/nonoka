...
mod reclaim_probe {
    ...
    ///
    /// ```
    /// cp ~/.nanoka/state/conversation.db /tmp/probe/
    /// NANOKA_RECLAIM_PROBE_DIR=/tmp/probe \
    ...
    fn reclaim_on_a_real_database() {
        let Some(dir) = std::env::var_os("NANOKA_RECLAIM_PROBE_DIR") else {
            println!("\n  跳过：没给 NANOKA_RECLAIM_PROBE_DIR");
            return;
        };
