...
/// checks before restoring a database written by a newer Hotaru.
...
/// Backs a one-shot `hotaru ask` / `hotaru '<message>'` turn: created just before
...
impl StateStore {
    pub fn new(paths: &HotaruPaths) -> Result<Self> {
    ...
    pub fn init_files(&self) -> Result<()> {
        ...
        if !self.profile_file().exists() {
            std::fs::write(self.profile_file(), "# Hotaru Profile\n\n")?;
        }
        Ok(())
    }
