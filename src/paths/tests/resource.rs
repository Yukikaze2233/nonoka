...
fn resource_path_remapping_includes_the_legacy_xdg_config_root() {
    ...
    let paths = NonokaPaths {
        ...
        fish_hook_file: base.config_dir().join("fish/conf.d/nonoka.fish"),
        bash_hook_file: root.join("config/shell/bash-hook.sh"),
        zsh_hook_file: root.join("config/shell/zsh-hook.zsh"),
        scripts_dir: root.join("data/scripts"),
        system_scripts_dir: PathBuf::from("/usr/share/nonoka/scripts"),
    };
    assert_eq!(
        paths.migrated_resource_path(&base.config_dir().join("nonoka/prompts/team")),
...
fn resource_migration_holds_runtime_exclusion_through_commit() {
    ...
    let runtime_dir = layout.state_dir.join("nonoka");
    fs::create_dir_all(&runtime_dir).unwrap();
