---
name: skill-creator
description: Create or safely update Hotaru skills. Use when the user asks to create, author, improve, or modify a reusable skill, SKILL.md workflow, or skill resources.
compatibility: Hotaru built-in skill authoring workflow
---
...
4. Use `manage_skill` with `action=create` for a new skill or `action=update` for an existing one. Use the returned absolute `skill_file` and `skill_dir`; never guess a Hotaru home path.
...
Optional standard fields are `license`, `compatibility`, `metadata`, and `allowed-tools`. `allowed-tools` is compatibility metadata only and never grants Hotaru permissions.
