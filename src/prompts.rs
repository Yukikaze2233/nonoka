...
include!(concat!(env!("OUT_DIR"), "/default_hotaru_prompt.rs"));
...
/// 默认 Hotaru 人格的内置防失忆提示(A/B 实测定稿文本)。用户在
/// hints/default.md 写了自己的内容时被覆盖。
pub fn default_hotaru_hint() -> String {
    decode_embedded_prompt(OBFUSCATED_DEFAULT_HOTARU_HINT)
}

/// 默认 Hotaru 人格的内置预设对话(begin_dialogs)。用户在
/// dialogs/default.md 写了自己的内容时被覆盖。
pub fn default_hotaru_dialogs() -> String {
    decode_embedded_prompt(OBFUSCATED_DEFAULT_HOTARU_DIALOGS)
}
