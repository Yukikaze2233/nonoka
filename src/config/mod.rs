...
pub struct AppConfig {
    ...
    /// 裸 `nonoka` 的默认模式:"normal" | "dev";空(默认)=打印带模式说明的
    /// 帮助,逼一次显式选择。`nonoka normal` / `nonoka dev` 子命令始终可用。
    #[serde(default)]
    ...
    /// 语音前端(`nonoka-voice` 进程):唤醒词、本地识别、听写、提示音。
    #[serde(default)]
    pub voice: VoiceConfig,
}

/// 语音功能。整套只在 `voice.enabled` 时由 daemon 拉起独立的 `nonoka-voice`
...
pub struct VoiceConfig {
    ...
    pub wake_boost: f32,
    /// 麦克风设备名(`nonoka-voice devices` 可列),null = 系统默认。
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct VoiceTtsConfig {
    ...
    /// 任一开启都会拉起 nonoka-voice(唤醒关闭时它只管播放,不开麦克风)。
    #[serde(default)]
    pub enabled: bool,
...
pub struct NotificationsConfig {
    #[serde(default = "default_true")]
    pub enabled: bool,
    /// Notify when a reply finishes and Nonoka is waiting on you again.
    #[serde(default = "default_true")]
