(() => {
  ...
  const state = {
    ...
    jobsStripOpen: localStorage.getItem("hotaru.web.jobsStripOpen") === "1",
    bootId: null,
    latestEventId: 0,
    lastEventId: 0,
    replayRunIds: null,
    ...
    persona: {
      name: "Hotaru",
      avatar_url: "/assets/hotaru-logo.png",
      board_image_url: "/assets/hotaruwallpaper.png",
  ...
  function setTheme(theme, persist = true) {
    ...
    if (persist) safeStorageSet("hotaru.web.theme", selected);
  }
  ...
  function setColorScheme(scheme, persist = true) {
    ...
    if (persist) safeStorageSet("hotaru.web.colorScheme", requested);
  }
  ...
  async function probeMatugenTheme() {
    ...
    setColorScheme(safeStorageGet("hotaru.web.colorScheme") || (state.matugenAvailable ? "matugen" : "madobe"), false);
  }
  ...
  function setChatFontSize(size, persist = true) {
    ...
    if (persist) safeStorageSet("hotaru.web.chatFontSize", selected);
  }
  ...
  function setReasoningExpanded(value, persist = true) {
    ...
    if (persist) safeStorageSet("hotaru.web.reasoningExpanded", String(state.reasoningExpanded));
  }
  ...
  function setToolExpanded(value, persist = true) {
    ...
    if (persist) safeStorageSet("hotaru.web.toolExpanded", String(state.toolExpanded));
  }
  ...
  function setSidebarCollapsed(collapsed, { automatic = false } = {}) {
    ...
    if (!automatic) safeStorageSet("hotaru.web.sidebarCollapsed", String(state.sidebarCollapsed));
    syncArtifactLayout?.();
  }
  ...
  function normalizePersona(value) {
    const name = String(value?.name || "").trim() || "Hotaru";
  ...
  function renderPromptCollection(kind, titleText, activePath) {
    ...
    defaultOption.value = "";
    defaultOption.textContent = kind === "personas" ? "Hotaru 默认人格" : "不使用用户身份";
  ...
  async function apiRequest(path, options = {}) {
    ...
    try {
      ...
      throw new ApiError("无法连接 Hotaru WebUI", 0);
    }
  ...
  function uploadComposerAttachment(item) {
    ...
    request.setRequestHeader("X-Hotaru-Filename", encodeURIComponent(item.file.name));
  ...
  function renderStageTodos(todos) {
    ...
    const card = state.stageTodos ? window.HotaruTodos?.renderList(state.stageTodos) : null;
    if (!card) {
      panel.hidden = true;
      return;
    }
  ...
  async function loadStageTodos(sessionId) {
    ...
    try {
      ...
      renderStageTodos(window.HotaruTodos?.normalize(payload?.todos) || null);
    } catch (_) {
  ...
  function createConversationMedia(asset, { eager = false } = {}) {
    ...
    const alt = String(source.alt || "").trim() || "Hotaru 生成的图片";
   ...
   * 默认展开/收起由本地偏好 hotaru.web.reasoningExpanded 决定,与 summary/full 无关。
   */
  ...
  function createAssistantMessage({
    ...
    for (const round of Array.isArray(toolRounds) ? toolRounds : []) {
      ...
      for (const call of Array.isArray(round?.calls) ? round.calls : []) {
        ...
        if (window.HotaruShared?.isShareTool(String(call?.name || ""))) {
          const shared = window.HotaruShared.renderCard(String(call?.output || ""));
  ...
  function renderConversation({ forceScroll = false } = {}) {
    ...
    window.HotaruCommands?.renderNotices(elements.timeline, state.viewSessionId);
  ...
  function createPersistedToolCard(call) {
    ...
    const todos = window.HotaruTodos?.isTodoTool(name) ? window.HotaruTodos.render(output) : null;
    if (todos) card.appendChild(todos);
    // 分享附件同理:文件卡片是交付物,直接出现在气泡里,点击即下载。
    const shared = window.HotaruShared?.isShareTool(name) ? window.HotaruShared.renderCard(output) : null;
  ...
  function handleToolEvent(name, live, data) {
    ...
    if (name === "tool.image") {
      ...
      if (ok && window.HotaruTodos?.isTodoTool(tool.name)
        && runSessionId(live.runId) === String(state.viewSessionId || "")) {
        renderStageTodos(window.HotaruTodos.parse(output));
      }
      // 与回看那份同构（`createPersistedToolCard`）：待办列表挂在签外面。
      // 只在这里画会让实时和刷新后长得不一样,那正是工具签之前踩过的坑。
      if (ok && window.HotaruTodos?.isTodoTool(tool.name)) {
        const todos = window.HotaruTodos.render(output);
        tool.card.querySelector(".todo-panel")?.remove();
        if (todos) tool.card.appendChild(todos);
      }
      // 分享附件同坑同修:实时完成时也要挂,否则只有刷新后才能看到卡片。
      if (ok && window.HotaruShared?.isShareTool(tool.name)) {
        const shared = window.HotaruShared.renderCard(output);
  ...
  function renderJobsStrip() {
    ...
    if (collapsible) {
      ...
      toggle.addEventListener("click", () => {
        ...
        localStorage.setItem("hotaru.web.jobsStripOpen", state.jobsStripOpen ? "1" : "0");
        renderJobsStrip();
      });
  ...
  function showBlockedState(unauthorized, message = "") {
    ...
    elements.blockedTitle.textContent = unauthorized ? "登录 Hotaru" : "无法载入 Hotaru WebUI";
  ...
  const VIEW_SESSION_KEY = "hotaru.web.viewSession";
  ...
  async function submitTurn() {
    ...
    if (window.HotaruCommands?.match(content)) {
      window.HotaruCommands.hide();
      ...
      let handled = false;
      try {
        handled = await window.HotaruCommands.tryRun(content, {
          apiRequest,
  ...
  function bindEvents() {
    ...
    elements.artifactResizeHandle.addEventListener("pointerdown", (event) => {
      ...
      const finish = () => {
        ...
        safeStorageSet("hotaru.web.artifactWidthRatio.v2", String(state.artifactWidthRatio));
    ...
    elements.composerInput.addEventListener("input", () => {
      window.HotaruCommands?.onInput(elements.composerInput.value, elements.composerDock, (name) => {
        elements.composerInput.value = name;
        elements.composerInput.focus();
        resizeComposer();
      });
    });
    elements.composerInput.addEventListener("blur", () => window.HotaruCommands?.hide());
    ...
    elements.composerInput.addEventListener("keydown", (event) => {
      ...
      // 与 REPL 一致，用户有机会反悔。
      if (window.HotaruCommands?.handleKey(event)) {
        event.preventDefault();
        return;
      }
  ...
  function initialize() {
    ...
    setTheme(safeStorageGet("hotaru.web.theme") || "graphite", false);
    const storedScheme = safeStorageGet("hotaru.web.colorScheme");
    if (storedScheme) setColorScheme(storedScheme, false);
    probeMatugenTheme();
    setChatFontSize(safeStorageGet("hotaru.web.chatFontSize") || "15px", false);
    setReasoningExpanded(safeStorageGet("hotaru.web.reasoningExpanded") === "true", false);
    setToolExpanded(safeStorageGet("hotaru.web.toolExpanded") === "true", false);
    const artifactRatio = Number(safeStorageGet("hotaru.web.artifactWidthRatio.v2"));
    if (Number.isFinite(artifactRatio) && artifactRatio >= 0.25 && artifactRatio <= 0.9) {
      state.artifactWidthRatio = artifactRatio;
    }
    setSidebarCollapsed(safeStorageGet("hotaru.web.sidebarCollapsed") === "true");
    syncArtifactLayout();
    ...
    // 所有 / 开头的输入照常发给模型。
    window.HotaruCommands?.load(apiRequest);
    // 灯箱自己不会画图标（图标集在这边），把工厂函数递过去。
    window.HotaruLightbox?.init({ makeIconSlot });
    startBrailleTicker();
    // G2:页面不可见时给 body 挂 hotaru-paused,CSS 据此暂停全部装饰动画。
    // 实测(Xvfb+Chrome)不挂这个时隐藏窗口的合成负载与可见时完全一样。
    const syncPaused = () => document.body.classList.toggle("hotaru-paused", document.hidden);
