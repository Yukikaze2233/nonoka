...
These static assets are embedded into the Nonoka daemon at build time. Run the local WebUI with:

```sh
cargo run --bin nonoka -- web
```

The command starts the Nonoka daemon (the same `nonoka` executable re-run in daemon mode) when needed, prints the access URLs, and exits. Use `nonoka daemon status` or `nonoka daemon stop` to inspect or stop the daemon. WebUI listens on all local network interfaces by default. Password protection is optional:

```sh
cargo run --bin nonoka -- web -p secret
cargo run --bin nonoka -- web -p
cargo run --bin nonoka -- web --password-file /path/to/password.txt
```
...
Two built-in themes derive from the Nonoka logo:
...
The 404 when no override exists is harmless. Serving `~/.nonoka/config/webui-theme.css`
